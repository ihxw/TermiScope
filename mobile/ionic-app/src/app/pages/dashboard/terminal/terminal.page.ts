...
export class TerminalPage implements OnInit, OnDestroy {
  ...
  async showQuickCommands() {
    ...
    const loading = await this.loadingController.create({
      message: await this.translate.get('terminal.commands').toPromise()
    });
    await loading.present();

    this.commandService.getCommands().pipe(
      finalize(() => loading.dismiss())
    ).subscribe({
      next: async (commands: Command[] = []) => {
        const cancelText = await this.translate.get('common.cancel').toPromise();
        const buttons: any[] = commands.map((command) => ({
          text: command.auto_enter ? `${command.name} ↵` : command.name,
          handler: () => {
        ...
        if (buttons.length === 0) {
          buttons.push({
            text: await this.translate.get('terminal.noCommandTemplates').toPromise(),
            role: 'cancel'
          });
        }

        buttons.push({ text: cancelText, role: 'cancel' });
