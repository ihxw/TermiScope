...
class AntdFileList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (files.isEmpty) {
      return Center(child: AntdEmpty(description: emptyText ?? '\u6682\u65e0\u6587\u4ef6'));
    }
    ...
    return ListView.separated(
      ...
      itemBuilder: (ctx, i) {
        ...
        return InkWell(
          ...
          child: Container(height: AntdTokens.tableRowHeight,
            padding: const EdgeInsets.symmetric(horizontal: 12),
            child: Row(children: [
              Icon(e.icon ?? (e.isDir ? Icons.folder : Icons.insert_drive_file),
                  size: 18,
                  color: e.isDir ? const Color(0xFFFAAD14) : const Color(0xFF8C8C8C)),
              const SizedBox(width: 10),
              Expanded(child: Text(e.name, overflow: TextOverflow.ellipsis,
                  style: TextStyle(fontSize: AntdTokens.fontSize,
                      fontWeight: e.isDir ? FontWeight.w600 : FontWeight.w400,
                      color: AntdTokens.textColor(context)))),
              if (e.size != null) ...[
                const SizedBox(width: 12),
                Text(_fmt(e.size!), style: TextStyle(fontSize: AntdTokens.fontSizeSM, color: sec)),
              ],
              if (e.modifiedTime != null) ...[
                const SizedBox(width: 12),
                Text(e.modifiedTime!, style: TextStyle(fontSize: AntdTokens.fontSizeSM, color: sec)),
              ],
              if (actions != null && onAction != null) ...[
                const SizedBox(width: 4),
                AntdActionMenu(items: actions!(e), onAction: (k) => onAction!.call(k, e)),
              ],
            ]),
          ),
        );
