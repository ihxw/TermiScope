...
class _SystemManagementScreenState extends State<SystemManagementScreen>
  ...
  void _triggerBackup() {
    ...
    showDialog(context: context, builder: (_) => AntdModal(
      title: Text(Translation.getText(context.read<AppState>().locale, 'system.backupTitle')),
      width: 420, okText: Translation.getText(context.read<AppState>().locale, 'common.confirm'),
      cancelText: Translation.getText(context.read<AppState>().locale, 'common.cancel'),
      onOk: () async {
        final res = await context.read<AppState>().backupDatabase(ctrl.text);
        if (res != null && mounted) _showBackupResult(res);
      },
      child: AntdPasswordInput(controller: ctrl, placeholder: '\u5907\u4efd\u5bc6\u7801\uff08\u53ef\u9009\uff09'),
    ));
  }

  void _showBackupResult(Map<String, dynamic> res) {
    showDialog(context: context, builder: (_) => AntdModal(
      title: const Text('\u5907\u4efd\u6210\u529f'), width: 400,
      okText: 'OK', cancelText: '', onOk: () {},
      child: Text('\u6587\u4ef6\u8def\u5f84: ${res['filepath']}\n\u5927\u5c0f: ${res['size'] ?? ''} bytes'),
    ));
  }
  ...
  void _pruneData() {
    ...
    showDialog(context: context, builder: (_) => AntdModal(
      title: Text(Translation.getText(st.locale, 'system.pruneConfirmTitle')),
      width: 420, danger: true,
      ...
      onOk: () async {
        ...
        if (res != null && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text('\u5df2\u6e05\u7406\uff01\u5220\u9664 ${res['deleted_count'] ?? 0} \u884c\u3002')));
          _loadAllData();
        }
      },
      child: Text(Translation.getText(st.locale, 'system.pruneConfirmContent')),
    ));
  }
  ...
  void _showAddEditTemplateDialog([Map<String, dynamic>? tmpl]) {
    ...
    final intervalC = TextEditingController(text: isEdit ? tmpl['interval']?.toString() : '60');
    final targetC = TextEditingController(text: isEdit ? tmpl['target'] : '8.8.8.8');
    final colorC = TextEditingController(text: isEdit ? tmpl['indicator_color'] : '#64D2FF');

    showDialog(context: context, builder: (_) => StatefulBuilder(builder: (ctx, setDlg) => AntdModal(
      title: Text(isEdit ? '\u7f16\u8f91\u6a21\u677f' : '\u65b0\u5efa\u6a21\u677f'), width: 480,
      okText: Translation.getText(context.read<AppState>().locale, 'common.confirm'),
      cancelText: Translation.getText(context.read<AppState>().locale, 'common.cancel'),
      onOk: () async {
        final data = {'name':nameC.text,'type':typeC.text,'interval':int.tryParse(intervalC.text)??60,'target':targetC.text,'indicator_color':colorC.text};
        final ok = isEdit ? await context.read<AppState>().updateNetworkTemplate(tmpl['id'] as int, data)
            : await context.read<AppState>().createNetworkTemplate(data);
        if (ok) _loadAllData();
      },
      child: Column(children: [
        AntdFormItem(label: '\u6a21\u677f\u540d\u79f0', child: AntdInput(controller: nameC)),
        const SizedBox(height: 12),
        AntdFormItem(label: '\u7c7b\u578b',
            child: AntdSelect<String>(value: typeC.text, options: const [
              AntdSelectOption(value: 'ping', label: 'Ping'),
              AntdSelectOption(value: 'tcping', label: 'TCPing'),
            ], onChanged: (v) { if (v != null) setDlg(() => typeC.text = v); })),
        const SizedBox(height: 12),
        AntdFormItem(label: '\u76ee\u6807 (IP/\u4e3b\u673a)', child: AntdInput(controller: targetC)),
        const SizedBox(height: 12),
        AntdFormItem(label: '\u9891\u7387 (\u79d2)', child: AntdInput(controller: intervalC, keyboardType: TextInputType.number)),
        const SizedBox(height: 12),
        AntdFormItem(label: '\u56fe\u8868\u989c\u8272', child: AntdInput(controller: colorC)),
      ]),
    )));
  }
  ...
  void _deployTemplate(Map<String, dynamic> tmpl) {
    ...
    final selected = <int>[];
    showDialog(context: context, builder: (_) => StatefulBuilder(builder: (ctx, setDlg) => AntdModal(
      title: Text('\u90e8\u7f72: ${tmpl['name']}'), width: 500, bodyMaxHeight: 300,
      okText: '\u90e8\u7f72', cancelText: Translation.getText(state.locale, 'common.cancel'),
      onOk: selected.isEmpty ? null : () async {
        final ok = await state.batchApplyNetworkTemplate(tmpl['id'] as int, selected);
        if (ok && mounted) ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('\u5ef6\u8fdf\u6a21\u677f\u5df2\u90e8\u7f72\uff01'), backgroundColor: Colors.green));
      },
      child: state.hosts.isEmpty ? const AntdEmpty(description: '\u6682\u65e0\u53ef\u7528\u4e3b\u673a')
          : ListView.builder(shrinkWrap: true, itemCount: state.hosts.length, itemBuilder: (_, i) {
              final h = state.hosts[i];
              final sel = selected.contains(h.id);
              return CheckboxListTile(title: Text(h.name), subtitle: Text(h.host),
                value: sel, onChanged: (v) => setDlg(() { if (v==true) selected.add(h.id); else selected.remove(h.id); }));
            }),
    )));
  }
  ...
  void _deleteTemplate(Map<String, dynamic> tmpl) {
    ...
    showDialog(context: context, builder: (_) => AntdModal(
      ...
      okText: Translation.getText(st.locale, 'common.confirm'),
      cancelText: Translation.getText(st.locale, 'common.cancel'),
      onOk: () async { final ok = await st.deleteNetworkTemplate(tmpl['id'] as int); if (ok) _loadAllData(); },
      child: Text('\u786e\u5b9a\u8981\u5220\u9664 ${tmpl['name']} \u5417\uff1f'),
    ));
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          AntdButton(type: AntdButtonType.primary, block: true, onPressed: _saveSettings,
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  AntdButton(onPressed: _testEmail,
                      child: Text(Translation.getText(state.locale, 'system.testEmail'))),
                ],
              ),
            ),
          ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  AntdButton(onPressed: _testTelegram,
                      child: Text(Translation.getText(state.locale, 'system.testTelegram'))),
                ],
              ),
            ),
          ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  AntdButton(type: AntdButtonType.primary, icon: Icons.backup,
                      onPressed: _triggerBackup,
                      child: Text(Translation.getText(state.locale, 'system.startBackup'))),
                ],
              ),
            ),
          ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  AntdButton(type: AntdButtonType.primary, icon: Icons.cleaning_services, danger: true,
                      onPressed: _pruneData,
                      child: Text(Translation.getText(state.locale, 'system.pruneMonitorData'))),
                ],
              ),
            ),
          ),
                ...
                return Card(
                  child: ListTile(
                    ...
                    trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                      AntdButton(type: AntdButtonType.text, icon: Icons.arrow_circle_right_outlined,
                          onPressed: () => _deployTemplate(tmpl)),
                      AntdButton(type: AntdButtonType.text, icon: Icons.edit,
                          onPressed: () => _showAddEditTemplateDialog(tmpl)),
                      AntdButton(type: AntdButtonType.text, icon: Icons.delete, danger: true,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      AntdTabs(items: const [
        AntdTabsItem(key: '0', label: Text('\u8bbe\u7f6e')),
        AntdTabsItem(key: '1', label: Text('\u901a\u77e5\u6e20\u9053')),
        AntdTabsItem(key: '2', label: Text('\u6570\u636e\u5e93\u7ef4\u62a4')),
        AntdTabsItem(key: '3', label: Text('\u5ef6\u8fdf\u6a21\u677f')),
      ], activeKey: _tabController.index.toString(), onChange: (k) => _tabController.animateTo(int.parse(k))),
