...
class _MonitorTemplatesScreenState extends State<MonitorTemplatesScreen> {
  ...
  void _showEditModal(Map<String, dynamic>? template) {
    ...
    final portCtrl = TextEditingController(text: (template?['port'] ?? 80).toString());
    final labelCtrl = TextEditingController(text: template?['label'] ?? '');
    final freqCtrl = TextEditingController(text: (template?['frequency'] ?? 60).toString());
    final colorCtrl = TextEditingController(text: template?['color'] ?? '#1890ff');
          ...
          builder: (context, setModalState) {
            return AntdModal(
              ...
              onOk: () async {
                ...
                bool ok;
                if (isEdit) {
                  ok = await state.updateNetworkTemplate(template['id'] as int, payload);
                } else {
                ...
                if (ok) {
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text(isEdit ? '模板更新成功' : '模板添加成功'), backgroundColor: AntdTokens.success),
                    );
                  }
                  _loadTemplates();
                } else {
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('操作失败'), backgroundColor: AntdTokens.error),
                    );
                  }
                }
              },
              child: SingleChildScrollView(
                child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                  AntdFormItem(
                    label: '模板名称',
                    required: true,
                    child: AntdInput(controller: nameCtrl, placeholder: 'e.g. Google DNS'),
                  ),
                  ...
                  AntdFormItem(
                    ...
                    child: DropdownButton<String>(
                      ...
                      items: const [
                        DropdownMenuItem(value: 'ping', child: Text('Ping (ICMP)')),
                        DropdownMenuItem(value: 'tcping', child: Text('TCPing (Port)')),
                      ],
                      onChanged: (v) {
                        if (v != null) {
                  ...
                  AntdFormItem(
                    label: '目标地址',
                    required: true,
                    child: AntdInput(controller: targetCtrl, placeholder: 'e.g. 8.8.8.8'),
                  ),
                  const SizedBox(height: 12),
                  if (selectedType == 'tcping') ...[
                    AntdFormItem(
                      label: '目标端口',
                      required: true,
                      child: AntdInput(controller: portCtrl, keyboardType: TextInputType.number),
                    ),
                    const SizedBox(height: 12),
                  ],
                  AntdFormItem(
                    label: '标签 (选填)',
                    child: AntdInput(controller: labelCtrl, placeholder: 'e.g. DNS'),
                  ),
                  const SizedBox(height: 12),
                  AntdFormItem(
                    label: '检测频率 (秒)',
                    child: AntdInput(controller: freqCtrl, keyboardType: TextInputType.number),
                  ),
                  const SizedBox(height: 12),
                  AntdFormItem(
                    label: '图表颜色',
                    child: Row(children: [
                      Expanded(
                        child: AntdInput(controller: colorCtrl, placeholder: '#1890ff'),
                      ),
                      const SizedBox(width: 8),
                      // Simple preset colors
                      ...['#F44336', '#4CAF50', '#1890ff', '#FFC107'].map((c) => GestureDetector(
                            onTap: () {
      ...
      builder: (_) => AntdModal(
        ...
        onOk: () async {
          ...
          if (ok) {
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('模板已删除'), backgroundColor: AntdTokens.success),
              );
            }
            _loadTemplates();
          } else {
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('删除失败'), backgroundColor: AntdTokens.error),
              );
            }
          }
        },
  ...
  Future<void> _openApplyModal(Map<String, dynamic> template) async {
    ...
    try {
      final res = await state.apiService.get('/api/monitor/network/templates/$templateId/assignments');
      if (res is List) {
        assignedHostIds = res.cast<int>();
      }
    } catch (e) {
      print('Load template assignments error: $e');
    }
        ...
        return StatefulBuilder(
          builder: (context, setModalState) {
            return AntdModal(
              ...
              cancelText: '取消',
              onOk: () async {
                final ok = await state.batchApplyNetworkTemplate(templateId, selectedHostIds.toList());
                if (ok) {
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('模板已成功部署到选定主机'), backgroundColor: AntdTokens.success),
                    );
                  }
                } else {
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('部署部署失败'), backgroundColor: AntdTokens.error),
                    );
                  }
                }
              },
  ...
  Widget build(BuildContext context) {
    ...
    final columns = <AntdTableColumn<Map<String, dynamic>>>[
      AntdTableColumn(
        title: '模板名称',
        width: 150,
        cell: (ctx, row, _) => Text(row['name'] ?? '', style: const TextStyle(fontWeight: FontWeight.bold)),
      ),
      AntdTableColumn(
        title: '检测类型',
        width: 100,
      ...
      AntdTableColumn(
        ...
        cell: (ctx, row, _) => Row(children: [
          ...
          TextButton(
            ...
            child: const Text('删除', style: TextStyle(color: AntdTokens.error, fontSize: 12)),
          ),
        ]),
      ),
    ];
