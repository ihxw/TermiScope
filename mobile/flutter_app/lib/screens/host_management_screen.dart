...
class _HostManagementScreenState extends State<HostManagementScreen> {
  ...
  void _test(AppState st, Host h) async {
    final r = st.isLocalNetworkHost(h.id)
  ...
  void _showInstallCommand(AppState st, Host h) {
    showDialog(context: context, builder: (_) => AntdModal(
      title: Text(_text(st, '手动安装命令', 'Manual install command')),
      width: 600,
      showFooter: false,
      child: SizedBox(height: 320, child: SingleChildScrollView(child: Text(
        'curl -fsSL \${baseUrl}/scripts/install.sh | sudo bash',
    ...
    final columns = <AntdTableColumn<Host>>[
      AntdTableColumn(
        ...
        cell: (ctx, h, _) => Row(children: [
          ...
          Icon(_osIcon(h.osType), size: 16, color: AntdTokens.textColor(context)),
        ...
        cell: (ctx, h, _) {
          ...
          return Row(mainAxisSize: MainAxisSize.min, children: [
            AntdDropdown<String>(items: [
              AntdDropdownItem(value: 'deploy', label: _text(st, '自动部署', 'Auto deploy'), icon: Icons.cloud_upload_outlined),
              AntdDropdownItem(value: 'manual', label: _text(st, '手动安装', 'Manual install'), icon: Icons.content_copy),
              if (h.deletedAt == null && h.monitorEnabled) AntdDropdownItem(value: 'stop', label: _text(st, '停止监控', 'Stop monitor'), icon: Icons.stop_circle_outlined, danger: true),
            ], onSelected: (key) {
              switch (key) {
                case 'deploy': _deploy(st, h);
                case 'manual': _showInstallCommand(st, h);
