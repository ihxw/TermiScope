...
class _TerminalSessionViewState extends State<TerminalSessionView> {
  ...
  void initState() {
    ...
    terminal.onOutput = (data) => _terminalService?.write(data);
  ...
  void _scheduleTerminalFlush() {
    if (_terminalOutputFlushScheduled) return;
    _terminalOutputFlushScheduled = true;
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => _flushPendingOutput(),
    );
  }

  void _flushPendingOutput() {
    _terminalOutputFlushScheduled = false;
    if (_disposed || _pendingTerminalOutput.isEmpty) return;

    // Limit per-frame processing to 64KB to avoid blocking the UI thread
    // during large bursts (e.g. cat a large file). Remaining data is
    // processed over subsequent frames.
    const maxChunkSize = 65536;

    final data = _pendingTerminalOutput.toString();
    _pendingTerminalOutput.clear();

    if (data.length > maxChunkSize) {
      _writeTerminalOutput(data.substring(0, maxChunkSize));
      _pendingTerminalOutput.write(data.substring(maxChunkSize));
      _scheduleTerminalFlush();
    } else {
      _writeTerminalOutput(data);
    }
  }
  ...
  Future<void> _showCommandTemplates() async {
    ...
    if (appState.commandTemplates.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('暂无命令模板')),
      );
      return;
    }
                ...
                child: GestureDetector(
                  ...
                  child: kIsWeb
                      ? WebTerminalView(
                          ...
                          onData: (data) => _terminalService?.write(data),
                          onResize: (cols, rows) {
    ...
    return ValueListenableBuilder<String>(
      ...
      builder: (context, status, child) {
        ...
        return Container(
          ...
          child: Row(
            children: [
              if (disconnected || _manualDisconnected)
              ...
              _statusButton(
                ...
                connected
                    ? () => ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Flutter 端 SFTP 分屏待接入')),
                        )
                    : null,
              ),
              _statusDivider(),
