...
class _TerminalTabsScreenState extends State<TerminalTabsScreen>
  ...
  Future<void> _handleHostSelected(AppState state, String hostId) async {
    final host = state.hosts.firstWhere((item) => hostId == '${item.id}');
    final existing = state.activeTerminals.firstWhere(
      (terminal) => '${terminal['hostId']}' == hostId,
      orElse: () => {},
    );
    if (existing.isEmpty) {
    ...
    final switchToExisting = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => AntdModal(
        title: Text(Translation.getText(
          state.locale,
          'terminal.hostAlreadyConnected',
        )),
        width: 440,
        okText: Translation.getText(state.locale, 'host.switchToExisting'),
        cancelText: Translation.getText(state.locale, 'host.openNew'),
        onOk: () => Navigator.of(dialogContext).pop(true),
        onCancel: () => Navigator.of(dialogContext).pop(false),
        child: Text(
          '${Translation.getText(state.locale, 'terminal.sessionAlreadyOpen')}\n${host.name}',
        ),
      ),
    );
    if (!mounted || switchToExisting == null) return;
    if (switchToExisting) {
      state.setActiveTabId(existing['tabId']?.toString());
    } else {
      state.addTerminal(
        host,
            ...
            children: [
              ...
              Expanded(
                child: terminals.isEmpty
                    ...
                    : _buildTerminalStack(terminals, state.activeTabId),
              ),
            ],
          ),
        );
  ...
  Widget _buildTerminalStack(
    ...
    final activeIndex = _activeTerminalIndex(terminals, activeTabId);
    return IndexedStack(
      index: activeIndex,
      children: [
        for (var index = 0; index < terminals.length; index++)
          TerminalSessionView(
            key: ValueKey(
              '${terminals[index]['tabId']}:${state.isLocalNetworkHost(int.parse(terminals[index]['hostId'].toString()))}',
            ),
            hostId: int.parse(terminals[index]['hostId'].toString()),
            hostLabel: terminals[index]['name']?.toString() ?? '',
            record: terminals[index]['record'] == true,
            active: index == activeIndex,
            localNetwork: state.isLocalNetworkHost(
              int.parse(terminals[index]['hostId'].toString()),
            ),
          ),
      ],
    );
