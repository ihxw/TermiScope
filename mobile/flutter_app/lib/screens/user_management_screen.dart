...
class _UserManagementScreenState extends State<UserManagementScreen> {
    ...
    showDialog(
        ...
        builder: (_) => StatefulBuilder(
            builder: (dialogContext, setDlg) => AntdModal(
                  title: Text(isEdit
                      ? Translation.getText(st.locale, 'user.editUser')
                      : Translation.getText(st.locale, 'user.addUser')),
                  width: 480,
                  ...
                  onOk: () async {
                    ...
                    final d = {
                      'username': name,
                      if (!isEdit) 'password': pw,
                      'display_name': disp.text.trim(),
                      'email': email.text.trim(),
                      'role': role
                    };
                    final ok = isEdit
                        ? await st.updateUser(u['id'] as int, d)
                        : await st.createUser(d);
                  ...
                  child: Column(children: [
                    AntdFormItem(
                        label: Translation.getText(st.locale, 'user.username'),
                        required: true,
                    ...
                    if (!isEdit)
                      AntdFormItem(
                          label:
                              Translation.getText(st.locale, 'auth.password'),
                          required: true,
                          child: AntdPasswordInput(controller: pass)),
                    if (!isEdit) const SizedBox(height: 12),
                    AntdFormItem(
                        label:
                            Translation.getText(st.locale, 'user.displayName'),
                        child: AntdInput(controller: disp)),
                    const SizedBox(height: 12),
                    AntdFormItem(
                        label: Translation.getText(st.locale, 'user.email'),
                        child: AntdInput(
                            controller: email,
                            keyboardType: TextInputType.emailAddress)),
                    const SizedBox(height: 12),
                    AntdFormItem(
                        label: Translation.getText(st.locale, 'user.role'),
  ...
  void _del(Map<String, dynamic> u) {
    ...
    showDialog(
        ...
        builder: (dialogContext) => AntdModal(
              ...
              cancelText: Translation.getText(st.locale, 'common.cancel'),
              onOk: () async {
                final ok = await st.deleteUser(u['id'] as int);
                if (ok && dialogContext.mounted) {
                  Navigator.of(dialogContext).pop();
                  _load();
                }
  ...
  Future<void> _toggle(Map<String, dynamic> u, bool cur) async {
    final ok =
        await context.read<AppState>().toggleUserStatus(u['id'] as int, !cur);
    if (ok) _load();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final cols = <AntdTableColumn<Map<String, dynamic>>>[
      AntdTableColumn(
        title: Translation.getText(st.locale, 'user.username'),
        width: 130,
        cell: (ctx, u, _) => Row(children: [
          CircleAvatar(
              radius: 12,
              backgroundColor:
                  (u['role'] == 'admin' ? AntdTokens.primary : Colors.grey)
                      .withAlpha(40),
              child: Icon(
                  u['role'] == 'admin'
      ...
      AntdTableColumn(
        title: Translation.getText(st.locale, 'user.displayName'),
        width: 100,
        cell: (ctx, u, _) => Text(u['display_name'] ?? '',
            overflow: TextOverflow.ellipsis,
            style: TextStyle(
                fontSize: AntdTokens.fontSizeSM,
                color: AntdTokens.secondaryTextColor(ctx))),
      ),
      AntdTableColumn(
        title: Translation.getText(st.locale, 'user.email'),
        width: 150,
        cell: (ctx, u, _) => Text(u['email'] ?? '',
            overflow: TextOverflow.ellipsis,
      ...
      AntdTableColumn(
        ...
        cell: (ctx, u, _) {
          ...
          return AntdSwitch(
              ...
              onChanged: (v) => _toggle(u, active));
        },
      ),
      AntdTableColumn(
        title: '\u64cd\u4f5c',
        width: 50,
        cell: (ctx, u, _) => AntdActionMenu(
            items: [
    ...
    return Column(children: [
      AntdToolbar(height: 44, bordered: true, leading: [
        Text(Translation.getText(st.locale, 'user.title'),
        ...
        child: Padding(
            ...
            child: Container(
                ...
                child: AntdTable<Map<String, dynamic>>(
                  ...
                  columns: cols,
                  emptyWidget:
                      const AntdEmpty(description: '\u6682\u65e0\u7528\u6237'),
                ))),
      ),
    ]);
  }
