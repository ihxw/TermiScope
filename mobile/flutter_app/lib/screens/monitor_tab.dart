...
class _MonitorTabState extends State<MonitorTab> {
  ...
  Widget _buildTable(
    ...
    final columns = <AntdTableColumn<Host>>[
                      ...
                      if (updateStatus.isNotEmpty) ...[
                        ...
                        AntdButton(
                            ...
                            onPressed: () async {
                              try {
                                await state
                                    .updateMonitorAgent(host.id.toString());
                              } catch (_) {}
                            },
                            child: Text(_text(state, '立即更新', 'Update now'),
                                style: const TextStyle(fontSize: 10)))
                      ]
                    ]),
                    Text(info['hostname']?.toString() ?? host.host,
                        overflow: TextOverflow.ellipsis,
                        style: const TextStyle(
        ...
        cell: (_, host, __) {
          ...
          return Column(
              ...
              children: [
                AntdTag(
                    preset:
                        online ? AntdTagPreset.success : AntdTagPreset.error,
                    label: online ? 'ONLINE' : 'OFFLINE'),
                if (online)
                  Text(_formatUptime(m.uptime),
                      style: const TextStyle(
                          fontSize: 12, color: const Color(0xFF8C8C8C))),
              ]);
        },
      ...
      _pcBarCol(
          ...
          (info, m) => (
                pct: m.cpu,
                label: '${m.cpu.toStringAsFixed(1)}%',
                detail: m.cpuCount > 0 ? '${m.cpuCount}C ${m.cpuModel}' : null
              )),
      _pcBarCol(
          state,
          'RAM',
          ...
          (info, m) => (
                pct: m.memoryPercent,
                label: '${_pct(m.memUsed, m.memTotal)}%',
                detail:
                    '${_formatBytes(m.memUsed)} / ${_formatBytes(m.memTotal)}'
              )),
      _tblDiskCol(state),
      _tblNetCol(state),
      ...
      AntdTableColumn(
        ...
        cell: (_, host, __) {
          ...
          final expColor = daysLeft < 0
              ? AntdTokens.error
              : (daysLeft <= 7 ? AntdTokens.warning : null);
          return Column(
              mainAxisAlignment: MainAxisAlignment.center,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                if (host.expirationDate != null)
                  Text.rich(TextSpan(children: [
                    TextSpan(
                        text: host.expirationDate!.split('T').first,
                        style: TextStyle(fontSize: 11, color: expColor)),
                    TextSpan(
                        text:
                            ' (${_text(state, '剩余 $daysLeft 天', '${daysLeft}d left')})',
                        style: TextStyle(
                            fontSize: 10,
                            color: expColor ?? const Color(0xFF8C8C8C))),
                  ])),
                if ((host.billingPeriod ?? '').isNotEmpty ||
                    host.billingAmount > 0)
                  Text(
                      '${_billingLabel(state, host.billingPeriod ?? '')}:${_currencySymbol(host.currency ?? '')}${host.billingAmount.toStringAsFixed(2)}',
                      style: const TextStyle(
                          fontSize: 10, color: const Color(0xFF8C8C8C))),
              ]);
        },
      ),
      AntdTableColumn(
        ...
        cell: (_, host, __) {
          ...
          return Row(mainAxisSize: MainAxisSize.min, children: [
            _iconBtn(
                Icons.show_chart, '', () => _openNetwork(host, 'connectivity')),
            _iconBtn(Icons.code, '',
                isMonitorOnly ? null : () => state.addTerminal(host)),
  ...
  AntdTableColumn<Host> _pcBarCol(
      AppState state,
      String title,
      double w,
      ({double pct, String label, String? detail}) Function(
              Map<String, dynamic>, _MonitorMetrics)
          fn) {
        ...
        cell: (_, host, __) {
          ...
          return Column(
              ...
              children: [
                ...
                AntdProgress(
                    percent: r.pct.clamp(0, 100).toDouble(),
                    color: _progressColor(r.pct),
                    strokeWidth: 4,
                    showInfo: false),
              ]);
        });
        ...
        cell: (_, host, __) {
          ...
          Widget content = Column(
              ...
              children: [
                Row(children: [
                  Text(
                      '${_formatBytes(m.diskUsed)} / ${_formatBytes(m.diskTotal)}',
                      style: const TextStyle(
                          fontSize: 10, color: const Color(0xFF8C8C8C))),
                  const Spacer(),
                  Text('${pct.toStringAsFixed(0)}%',
                      style:
                          TextStyle(fontSize: 11, color: _progressColor(pct))),
                ]),
                const SizedBox(height: 2),
                AntdProgress(
          ...
          return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Row(children: [
              const Icon(Icons.arrow_downward,
                  size: 12, color: AntdTokens.success),
              Expanded(
                  child: Text(' ${_formatBytes(m.rxRate)}/s',
                      style: const TextStyle(
                          fontSize: 12, color: AntdTokens.success))),
              Text(_formatBytes(m.rxTotal),
                  style: const TextStyle(
                      fontSize: 10, color: const Color(0xFF8C8C8C))),
            ]),
            Row(children: [
              const Icon(Icons.arrow_upward,
                  size: 12, color: AntdTokens.primary),
              Expanded(
                  child: Text(' ${_formatBytes(m.txRate)}/s',
                      style: const TextStyle(
                          fontSize: 12, color: AntdTokens.primary))),
              Text(_formatBytes(m.txTotal),
                  style: const TextStyle(
                      fontSize: 10, color: const Color(0xFF8C8C8C))),
            ]),
          ]);
        ...
        cell: (_, host, __) {
          ...
          return Column(
              ...
              children: [
                Row(children: [
                  Text(
                      '${_trafficLabel(state, host.netTrafficCounterMode ?? '')} · ${pct.toStringAsFixed(0)}%',
                      style: const TextStyle(
                          fontSize: 10, color: const Color(0xFF8C8C8C))),
                  const Spacer(),
                ...
                if ((host.netResetDay ?? 0) > 0)
                  Text(
                      '${_text(state, '下次重置', 'Next reset')}: ${_nextResetDate(host.netResetDay ?? 1)}',
                      style:
  ...
  Widget _buildMonitorCard(AppState state, Host host) {
    ...
    Widget card = AntdCard(
      ...
      extra: Row(mainAxisSize: MainAxisSize.min, children: [
        _iconBtn(Icons.show_chart, _text(state, '网络详情', 'Network'),
            () => _openNetwork(host, 'connectivity')),
        _iconBtn(
            Icons.code,
            isMonitorOnly
                ? _text(state, '仅监控', 'Monitor only')
                : _text(state, '终端', 'Terminal'),
            isMonitorOnly ? null : () => state.addTerminal(host)),
        _iconBtn(Icons.history, _text(state, '历史', 'History'),
            () => _showStatusLogs(state, host)),
        _iconBtn(Icons.settings, _text(state, '设置', 'Settings'),
            () => _openNetwork(host, 'config')),
      ]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(
            ...
            children: [
              ...
              const Text('|',
                  style:
                      TextStyle(fontSize: 12, color: const Color(0xFF8C8C8C))),
              Text(
                  '${_text(state, '运行时间', 'Uptime')}: ${_formatUptime(metrics.uptime)}',
                  style: const TextStyle(
                      fontSize: 12, color: const Color(0xFF8C8C8C))),
              if (version.isNotEmpty) ...[
                ...
                if (updateStatus.isNotEmpty)
                  ...
                  GestureDetector(
                    onTap: () async {
                      try {
                        await state.updateMonitorAgent(host.id.toString());
                      } catch (_) {}
                    },
                    child: AntdTag(
                        preset: AntdTagPreset.warning,
                        label: _text(state, '更新', 'Update')),
                  ),
              ],
            ]),
        ...
        Row(children: [
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                Text('↓ ${_formatBytes(metrics.rxRate)}/s',
                    style: const TextStyle(
                        color: AntdTokens.success, fontSize: 12)),
                Text(
                    '${_text(state, '总计', 'Total')}: ${_formatBytes(metrics.rxTotal)}',
                    style: const TextStyle(
                        color: const Color(0xFF8C8C8C), fontSize: 11)),
              ])),
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                Text('↑ ${_formatBytes(metrics.txRate)}/s',
                    style: const TextStyle(
                        color: AntdTokens.primary, fontSize: 12)),
                Text(
                    '${_text(state, '总计', 'Total')}: ${_formatBytes(metrics.txTotal)}',
                    style: const TextStyle(
                        color: const Color(0xFF8C8C8C), fontSize: 11)),
              ])),
        ]),
        ...
        if (limit > 0) ...[
          ...
          Row(children: [
            Expanded(
              flex: 3,
              child: Text(
                  '${_text(state, '流量', 'Usage')} · ${_trafficLabel(state, host.netTrafficCounterMode ?? '')} (${trafficPercent.toStringAsFixed(0)}%)',
                  overflow: TextOverflow.ellipsis,
            ...
            Expanded(
              flex: 2,
              child: Text(
                  '${_formatBytes(trafficUsed)} / ${_formatBytes(limit)}',
                  overflow: TextOverflow.ellipsis,
          ...
          const SizedBox(height: 2),
          AntdProgress(
              percent: trafficPercent.clamp(0, 100).toDouble(),
              color: _progressColor(trafficPercent),
              strokeWidth: 6,
              showInfo: false),
          if ((host.netResetDay ?? 0) > 0) ...[
            const SizedBox(height: 2),
    ...
    if (flagBorder != null) {
      card =
          Container(decoration: BoxDecoration(border: flagBorder), child: card);
    }
    if (!online) {
      card = ColorFiltered(
  ...
  Widget _sectionRow(String label, String pctStr,
      {String? subtitle, required double percent}) {
        ...
        if (subtitle != null)
          Flexible(
              child: Text(' $subtitle',
                  overflow: TextOverflow.ellipsis,
                  style: const TextStyle(
                      fontSize: 11, color: const Color(0xFF8C8C8C)))),
        const Spacer(),
        Text(pctStr,
            style: TextStyle(fontSize: 12, color: _progressColor(percent))),
      ]),
      const SizedBox(height: 2),
      AntdProgress(
  ...
  Widget _financialRow(AppState state, Host host) {
    ...
    final expColor = daysLeft < 0
        ...
        : (daysLeft <= 7 ? AntdTokens.warning : null);
    final hasBilling =
        (host.billingPeriod ?? '').isNotEmpty || host.billingAmount > 0;
    return Row(children: [
      ...
      if (host.expirationDate != null)
        Expanded(
          ...
          child: Text(
              ...
              overflow: TextOverflow.ellipsis,
              style: TextStyle(
                  fontSize: 11, color: expColor ?? const Color(0xFF8C8C8C))),
        ),
  ...
  String _trafficLabel(AppState state, String mode) {
    switch (mode) {
      case 'rx':
        return _text(state, '下载', 'RX');
      case 'tx':
        return _text(state, '上传', 'TX');
      default:
        return _text(state, '总计', 'Total');
    }
  }
  ...
  String _billingLabel(AppState state, String period) {
    switch (period.toLowerCase()) {
      case 'monthly':
        return _text(state, '月付', 'Monthly');
      case 'quarterly':
        return _text(state, '季付', 'Quarterly');
      case 'semiannually':
        return _text(state, '半年付', 'Semi-annual');
      case 'annually':
        return _text(state, '年付', 'Annual');
      default:
        return period;
    }
  }
