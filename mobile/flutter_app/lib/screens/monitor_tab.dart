    ...
    Widget card = AntdCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (limit > 0) ...[
          ...
          const SizedBox(height: 6),
          Row(children: [
            Text('${_text(state, '流量', 'Usage')} · ${_trafficLabel(state, host.netTrafficCounterMode ?? '')} (${trafficPercent.toStringAsFixed(0)}%)', style: const TextStyle(fontSize: 12)),
            const Spacer(),
            Text('${_formatBytes(trafficUsed)} / ${_formatBytes(limit)}', style: const TextStyle(fontSize: 12)),
          ]),
          const SizedBox(height: 2),
  ...
  Widget _financialRow(AppState state, Host host) {
    ...
    return Row(children: [
      ...
      if (host.expirationDate != null)
        Text('${_text(state, '到期', 'Expires')}: ${host.expirationDate!.split('T').first}',
            style: TextStyle(fontSize: 11, color: expColor ?? const Color(0xFF8C8C8C))),
      const SizedBox(width: 4),
      if (host.expirationDate != null)
        Text('(${_text(state, '剩余 $daysLeft 天', '${daysLeft}d left')})',
            style: TextStyle(fontSize: 10, color: expColor ?? const Color(0xFF8C8C8C))),
      const Spacer(),
      if ((host.billingPeriod ?? '').isNotEmpty || host.billingAmount > 0)
        Text('${_billingLabel(state, host.billingPeriod ?? '')}:${_currencySymbol(host.currency ?? '')}${host.billingAmount.toStringAsFixed(2)}',
