...
class _NetworkDetailScreenState extends State<NetworkDetailScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadStats() async {
    ...
    final state = context.read<AppState>();
    try {
      final res = await state.apiService.get('/api/ssh-hosts/${widget.hostId}/network/latency-stats?range=$_timeRange');
      if (res is Map && mounted) {
        ...
        for (final s in seriesList) {
          ...
          final points = data.map((item) {
            ...
            final failed = m['success'] == false || (m['latency'] is num && m['latency'] < 0);
            final double? lat = failed ? null : (m['latency'] as num?)?.toDouble();
        ...
        if (minTime != null && maxTime != null && minTime != maxTime) {
          final duration = maxTime!.difference(minTime!).inMilliseconds;
          for (final s in parsedSeries) {
            final List<Offset> pts = [];
            for (final pt in s) {
              final time = pt['time'] as DateTime;
              final lat = pt['latency'] as double?;
              final double dx = (time.difference(minTime!).inMilliseconds) / duration;
          ...
          // X Labels (3 points)
          final format = _timeRange.endsWith('d') ? 'MM-dd' : 'HH:mm';
          ...
          _xAxisLabels = [
            _fmt(minTime!),
            _fmt(minTime!.add(Duration(milliseconds: duration ~/ 2))),
            _fmt(maxTime!),
          ];
  ...
  Future<void> _saveConfig() async {
    ...
    try {
      ...
      final double trafficAdj = resetTraffic ? targetTotalBytes : targetTotalBytes - measuredTraffic;
      ...
      if (ok) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('网卡配置保存成功'), backgroundColor: AntdTokens.success),
          );
        }
        _loadAll();
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('保存配置失败'), backgroundColor: AntdTokens.error),
          );
        }
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('保存发生错误: $e'), backgroundColor: AntdTokens.error),
        );
      }
    } finally {
  ...
  Future<void> _saveNotifyConfig() async {
    ...
    try {
      ...
      if (ok) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('告警通知配置保存成功'), backgroundColor: AntdTokens.success),
          );
        }
        _loadAll();
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('保存告警配置失败'), backgroundColor: AntdTokens.error),
          );
        }
      }
    } catch (e) {
      ...
      child: Column(children: [
        AntdCard(
          ...
          extra: Row(children: [
            ...
            AntdButton(
              size: AntdSize.small,
              type: _isSmooth ? AntdButtonType.primary : AntdButtonType.defaultType,
            ...
            DropdownButton<String>(
              ...
              style: TextStyle(color: AntdTokens.textColor(context), fontSize: 12),
  ...
  Widget _buildConfigTab(BuildContext context) {
    ...
    final double limitBytes = _limitGb * 1024 * 1024 * 1024;
    final double remainingBytes = limitBytes - totalUsedBytes > 0 ? limitBytes - totalUsedBytes : 0;
    final int usagePct = limitBytes > 0 ? ((totalUsedBytes / limitBytes) * 100).round().clamp(0, 100) : 0;
        ...
        AntdCard(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            AntdFormItem(
              ...
              child: AntdInput(
                controller: TextEditingController(text: _selectedInterfaces.join(',')),
            ...
            AntdFormItem(
              ...
              child: AntdInput(
                controller: TextEditingController(text: _limitGb.toStringAsFixed(2)),
                placeholder: '0.00',
            ...
            AntdFormItem(
              ...
              child: AntdInput(
                controller: TextEditingController(text: _customTotalGb.toStringAsFixed(2)),
                placeholder: '0.00',
                keyboardType: const TextInputType.numberWithOptions(decimal: true),
            ...
            Text('月度流量监控报表', style: TextStyle(fontSize: AntdTokens.fontSizeLG, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(
                child: Column(children: [
                  const Text('流入 (Inbound)', style: TextStyle(fontSize: 12, color: Colors.grey)),
                  const SizedBox(height: 4),
                  Text(_formatBytes(_monthlyRx), style: const TextStyle(color: AntdTokens.success, fontSize: 16, fontWeight: FontWeight.bold)),
                ]),
              ),
              Expanded(
                child: Column(children: [
                  const Text('流出 (Outbound)', style: TextStyle(fontSize: 12, color: Colors.grey)),
                  const SizedBox(height: 4),
                  Text(_formatBytes(_monthlyTx), style: const TextStyle(color: AntdTokens.error, fontSize: 16, fontWeight: FontWeight.bold)),
                ]),
              ),
            ]),
            if (_limitGb > 0) ...[
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text('使用进度 ($usagePct%)', style: const TextStyle(fontSize: 12)),
                  Text('${_formatBytes(totalUsedBytes)} / ${_limitGb.toStringAsFixed(0)} GB', style: const TextStyle(fontSize: 12)),
                ]),
                const SizedBox(height: 6),
                AntdProgress(percent: usagePct.toDouble(), strokeWidth: 8, color: usagePct >= 90 ? AntdTokens.error : AntdTokens.primary),
                const SizedBox(height: 6),
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text('剩余配额: ${_formatBytes(remainingBytes)}', style: const TextStyle(fontSize: 11, color: Colors.grey)),
                  Text('重置周期: 每月 $_resetDay 号', style: const TextStyle(fontSize: 11, color: Colors.grey)),
                ]),
              ]),
            ],
          ]),
      ...
      AntdTableColumn(
        ...
        cell: (ctx, row, _) {
          ...
          return Column(
            ...
            children: ips.map((ip) => Text(ip.toString(), style: const TextStyle(fontSize: 11))).toList(),
          );
        },
      ),
      AntdTableColumn(
        ...
        cell: (ctx, row, _) => Row(children: [
          ...
          Text(_formatSpeed((row['rx_rate'] ?? 0).toDouble()), style: const TextStyle(color: AntdTokens.success, fontSize: 11)),
          const SizedBox(width: 8),
          const Icon(Icons.arrow_upward, size: 12, color: AntdTokens.primary),
          Text(_formatSpeed((row['tx_rate'] ?? 0).toDouble()), style: const TextStyle(color: AntdTokens.primary, fontSize: 11)),
        ]),
      ),
      AntdTableColumn(
        title: '总流量',
        ...
        cell: (ctx, row, _) => Column(
          ...
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Text('Rx: ${_formatBytes((row['rx'] ?? 0).toDouble())}', style: const TextStyle(fontSize: 11)),
            Text('Tx: ${_formatBytes((row['tx'] ?? 0).toDouble())}', style: const TextStyle(fontSize: 11)),
          ],
        ),
      ),
      AntdTableColumn(
        title: 'MAC 地址',
        width: 140,
        cell: (ctx, row, _) => Text(row['mac'] ?? '--', style: const TextStyle(fontFamily: 'monospace', fontSize: 11)),
      ),
    ];
      ...
      child: Column(children: [
        AntdCard(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Text('离线宕机通知', style: TextStyle(fontWeight: FontWeight.bold)),
              AntdSwitch(
            ...
            if (_notifyOfflineEnabled) ...[
              AntdFormItem(
                label: '连续离线阈值 (分钟)',
                child: AntdInput(
                  controller: TextEditingController(text: _notifyOfflineThreshold.toString()),
                  keyboardType: TextInputType.number,
                  onChanged: (v) => setState(() => _notifyOfflineThreshold = int.tryParse(v) ?? 1),
                ),
              ),
              const SizedBox(height: 16),
            ],
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Text('流量限额提醒通知', style: TextStyle(fontWeight: FontWeight.bold)),
              AntdSwitch(
            ...
            if (_notifyTrafficEnabled) ...[
              AntdFormItem(
                ...
                child: Slider(
                  ...
                  onChanged: (v) => setState(() => _notifyTrafficThreshold = v.toInt()),
                ),
              ),
              const SizedBox(height: 16),
            ],
            ...
            const SizedBox(height: 12),
            // Channel Checklist
            const Text('通知发送渠道 (支持多选)', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            CheckboxListTile(
...
class AntdLineChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final gridColor = isDark ? const Color(0xFF303030) : const Color(0xFFF0F0F0);
...
class _LineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < xCount; i++) {
      ...
      textPainter.layout();
      textPainter.paint(canvas, Offset(x - textPainter.width / 2, size.height - bottomMargin + 6));
    }
