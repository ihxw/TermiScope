...
class TerminalService implements TerminalConnection {
        ...
        (data) {
          if (data is String) {
            try {
              ...
              if (msg['type'] == 'error' &&
                  msg['code'] == 'fingerprint_mismatch') {
                onStatusChanged?.call('Error');
                onData?.call('\r\n[ERROR] ${msg['data']}\r\n');
                final fingerprint =
                    msg['meta']?['new_fingerprint']?.toString() ?? '';
                if (fingerprint.isNotEmpty) {
                  onFingerprintMismatch?.call(fingerprint);
                }
              } else if (msg['type'] == 'connected' || msg['type'] == 'error') {
                if (msg['type'] == 'error') onStatusChanged?.call('Error');
                onData?.call(
