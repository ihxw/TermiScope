...
class AppState extends ChangeNotifier {
  ...
  Future<void> fetchHosts({bool includeDeleted = false}) async {
    try {
      final data = await apiService.get(
        includeDeleted
  ...
  Future<Map<String, dynamic>> testHostConnection(String id) async {
    try {
      ...
      if (hostId != null) hostConnectionStatus[hostId] = true;
      notifyListeners();
      return {
        'success': true,
      ...
      final hostId = int.tryParse(id);
      if (hostId != null) hostConnectionStatus[hostId] = false;
      notifyListeners();
  ...
  Future<Map<String, dynamic>> revealHostCredentials(
    int hostId,
    String currentPassword,
  ) async {
    final encoded = Uri.encodeQueryComponent(currentPassword);
    final data = await apiService.get(
      '/api/ssh-hosts/$hostId?reveal=true&current_password=$encoded',
    );
  ...
  Future<Map<String, int>> deployMonitorAgents(Iterable<int> ids) async {
    return _runHostBatch(ids, (id) => deployMonitorAgent('$id'));
  }
