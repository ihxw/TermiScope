...
| `MonitorDashboard.vue` | `monitor_tab.dart` | 基本对齐 | 卡片/真实表格模式、断线提示、Agent 更新、CPU/磁盘明细、流量/财务、排序、分页状态历史和流量重置日志已接入 |
...
| `FileTransfer.vue` + SFTP components | `file_transfer_screen.dart` + `sftp_service.dart` | 基本对齐 | 双主机面板、上传/下载、文本编辑、单项/批量跨主机传输、冲突处理、进度 Dock、服务器历史/收藏已接入；媒体预览、剪切/复制/粘贴、拖放上传、任务取消/重试待补 |

## 下一批优先级

1. SFTP 补媒体预览、同主机剪切/复制/粘贴、拖放上传、任务取消与重试。
2. 终端页继续走查录制会话和移动端虚拟键盘的边界状态。
3. 监控页在真实后端数据下继续做卡片高度和窄屏表格的像素级校验。
4. 主机管理继续做桌面/移动端排序和超长主机名的视觉走查。
5. 公共流程继续做中英文文案和像素级视觉走查。
