...
- Dashboard 外壳：桌面端顶部横向导航，移动端抽屉，管理员菜单控制，后端版本与用户名显示。
...
- SFTP `file_transfer_screen.dart` 面包屑栏迁至 `AntdToolbar`+`AntdButton`+`AntdDropdown`，文件列表迁至 `AntdFileList`（目录/文件图标、大小/时间、行操作菜单），跳转路径/新建/重命名/删除弹窗全部迁至 `AntdModal`。
...
| `Dashboard.vue` | `home_screen.dart` | 基本对齐 | 顶部导航/下拉用户菜单已迁移到 AntdDropdown/AntdButton；更新检测/升级弹窗尚未移植 |
| `Terminal.vue` | `terminal_tabs_screen.dart` + `terminal_session_screen.dart` | 基本对齐 | 工具栏已迁移到 AntdToolbar/AntdSelect/AntdSwitch；tab 已迁移到 AntdTabs(editor)；命令模板底部弹窗已 antd 化；终端主题、指纹变更确认、重复会话确认待继续 |
| `components/Terminal.vue` | `terminal_session_screen.dart` | 部分对齐 | 虚拟键盘保留 Material 定制风格，不与 Antd 冲突 |
| `MonitorDashboard.vue` | `monitor_tab.dart` | 基本对齐 | 已添加卡片/列表模式切换，卡片迁至 AntdMetricCard（CPU/RAM/DISK 用 AntdProgress），每卡配操作按钮（网络/历史/设置）；状态历史弹窗/流量重置日志/模板入口/批量更新待补 |
| `NetworkDetail.vue` | 无 | 缺失 | 网络详情与配置页缺失 |
| `monitor/MonitorTemplates.vue` | 无 | 缺失 | 监控模板管理缺失 |
| `HostManagement.vue` | `host_management_screen.dart` + `host_edit_dialog.dart` | 基本对齐 | 已从卡片列表迁为 AntdTable 表格；搜索/过滤/操作列/状态圆点/删除确认已对齐 Web；批量选择与拖拽排序待补 |
...
| `FileTransfer.vue` + SFTP components | `file_transfer_screen.dart` | 基本对齐 | 面包屑+文件列表+右键操作+弹窗已改用 Antd 组件；上传队列/冲突弹窗/进度 Dock 组件已就绪（`AntdUploadProgressDock`/`AntdConflictModal`），待接入实际上传 API |

## 下一批优先级

1. AntdFlutter 阶段 2：实现 `AntdToolbar`、`AntdTabs`、`AntdDropdown`、`AntdModal`、`AntdSwitch`，迁移 `home_screen.dart`、`terminal_tabs_screen.dart`、`terminal_session_screen.dart`、`host_edit_dialog.dart`。
2. 终端页继续对齐：重复会话确认、终端主题/字体、指纹变更确认、SFTP 入口。
3. AntdFlutter 阶段 3：实现 `AntdTable`、`AntdPagination`、`AntdActionMenu`、`AntdStatusBadge`，把主机管理从卡片列表迁到表格结构。
4. 监控页补卡片/列表切换、每卡操作按钮、历史弹窗、重置日志、模板入口。
5. SFTP 文件传输补上传队列、冲突处理、文件编辑器与进度 Dock。
6. 公共流程进一步精修：语言文案完全对齐，逐步把 setup/login/forgot/reset 与 Web 视觉做最终走查。
