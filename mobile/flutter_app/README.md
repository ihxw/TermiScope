...
Windows x86_64 release build (run on Windows with Visual Studio Desktop development with C++ installed):
...
Changes under `mobile/flutter_app/` also trigger the `Flutter Windows x86_64`
GitHub Actions workflow. It publishes a
`TermiScope-Flutter-Windows-x86_64-v<version>.zip` artifact for 30 days and a
publicly downloadable prerelease asset tagged `flutter-v<version>`.
