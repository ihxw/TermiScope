...
func (h *MonitorHandler) Pulse(c *gin.Context) {
	...
	// 濡傛灉鐩戞帶鏈惎鐢紝鑷姩鍚敤锛堥娆℃敹鍒版暟鎹椂锛?	if !host.MonitorEnabled {
	...
	if host.Status != "online" {
		...
		// Check if this was a short outage (< 1 minute) 鈥?suppress notifications
...
func (h *MonitorHandler) Deploy(c *gin.Context) {
	...
	// 鐢熸垚鎴栦娇鐢ㄧ幇鏈夌殑 Secret锛堟案涔卼oken锛?	secret := host.MonitorSecret
	if secret == "" {
		// 鐢熸垚鏂扮殑 Secret
...
`, execCmd)
	...
	case "upstart":
		// Upstart service (Ubuntu 14.04 绛?
...
`, execCmd)
	...
	case "sysv":
		// SysV init script (CentOS 6, Debian 7 绛?
...
// BatchDeploy - 鎵归噺閮ㄧ讲agent鍒板涓富鏈?func (h *MonitorHandler) BatchDeploy(c *gin.Context) {
	...
	for _, hostID := range req.HostIDs {
		...
		go func(id uint) {
			defer wg.Done()
			semaphore <- struct{}{}        // 鑾峰彇淇″彿閲?			defer func() { <-semaphore }() // 閲婃斁淇″彿閲?
...
// deployToHost - 閮ㄧ讲agent鍒版寚瀹氫富鏈?浠嶥eploy鍑芥暟鎻愬彇鐨勬牳蹇冮€昏緫)
func (h *MonitorHandler) deployToHost(host *models.SSHHost, insecure bool, request *http.Request) error {
	...
	if err != nil {
		return fmt.Errorf("SSH杩炴帴澶辫触: %v", err)
	}
	defer client.Close()
	...
	session.Close()
	if err != nil {
		return fmt.Errorf("妫€娴嬫灦鏋勫け璐? %v", err)
	}
	arch := string(bytes.TrimSpace(output))
	...
	case "armv7l", "armv7":
		goArch = "arm"
	default:
		return fmt.Errorf("涓嶆敮鎸佺殑鏋舵瀯: %s", arch)
	}
	...
	binaryContent, err := ioutil.ReadFile(localBinaryPath)
	if err != nil {
		return fmt.Errorf("Agent浜岃繘鍒舵枃浠朵笉瀛樺湪: %s", goArch)
	}
	...
	if out, err := session.CombinedOutput(setupCmd); err != nil {
		return fmt.Errorf("鍒涘缓鐩綍澶辫触: %s", string(out))
	}
	session.Close()
	...
	if host.Username != "root" {
		...
		session.Stdin = strings.NewReader(password + "\n")
		if out, err := session.CombinedOutput(moveCmd); err != nil {
			return fmt.Errorf("绉诲姩鏂囦欢澶辫触: %s", string(out))
		}
		session.Close()
	}
	...
	if out, err := session.CombinedOutput(chmodCmd); err != nil {
		return fmt.Errorf("璁剧疆鏉冮檺澶辫触: %s", string(out))
	}
	session.Close()
...
// BatchStopRequest - 鎵归噺鍋滄鐩戞帶璇锋眰
type BatchStopRequest struct {
	HostIDs []uint `json:"host_ids" binding:"required"`
}

// BatchStopResult - 鍗曚釜涓绘満鍋滄缁撴灉
...
// stopMonitorOnHost - 鍋滄鎸囧畾涓绘満鐨勭洃鎺?浠嶴top鍑芥暟鎻愬彇鐨勬牳蹇冮€昏緫)
func (h *MonitorHandler) stopMonitorOnHost(host *models.SSHHost) error {
	...
	client, err := ssh.Dial("tcp", fmt.Sprintf("%s:%d", host.Host, host.Port), sshConfig)
	if err != nil {
		// SSH杩炴帴澶辫触涓嶇畻閿欒,鍥犱负DB宸茬粡鏇存柊
		log.Printf("Monitor BatchStop: SSH杩炴帴澶辫触(宸叉洿鏂癉B): %v", err)
		return nil
	}
	defer client.Close()
...
// GetInstallScript 鐢熸垚涓€閿畨瑁呰剼鏈?func (h *MonitorHandler) GetInstallScript(c *gin.Context) {
	hostID := c.Query("host_id")
	...
	// 鐢熸垚鎴栦娇鐢ㄧ幇鏈夌殑 Secret锛堣繖閲?secret 宸茬粡楠岃瘉杩囷紝鐩存帴浣跨敤锛?	if host.MonitorSecret == "" {
		// 杩欑鎯呭喌鐞嗚涓婁笉浼氬彂鐢燂紝鍥犱负涓婇潰宸茬粡楠岃瘉浜?		randomBytes := make([]byte, 32)
		rand.Read(randomBytes)
	...
	// 寮哄埗杞崲 CRLF 涓?LF锛岃В鍐?Windows/Linux 鎹㈣绗﹀吋瀹规€ч棶棰?	// For Windows PowerShell, CRLF is fine, but LF is also fine.
	if osType != "windows" {
...
// GetUninstallScript 鐢熸垚鍗歌浇鑴氭湰
func (h *MonitorHandler) GetUninstallScript(c *gin.Context) {
	// 鑾峰彇鍙傛暟锛堝鏋滈€氳繃鍓嶇鐢熸垚鐨勯摼鎺ヨ闂級
	secret := c.Query("secret")
	hostID := c.Query("host_id")
...
func (h *MonitorHandler) UninstallCallback(c *gin.Context) {
	...
	// 鏇存柊鐘舵€佷负鏈惎鐢ㄧ洃鎺?	h.DB.Model(&host).Update("monitor_enabled", false)
...
// DownloadAgent 鎻愪緵 agent 浜岃繘鍒舵枃浠朵笅杞?func (h *MonitorHandler) DownloadAgent(c *gin.Context) {
	...
	// 妫€鏌ユ枃浠舵槸鍚﹀瓨鍦紙浣跨敤 Stat 閬垮厤灏嗗ぇ鏂囦欢璇诲叆鍐呭瓨锛?	if info, err := os.Stat(filePath); err != nil {
	...
	// 璁剧疆鍝嶅簲澶?	c.Header("Content-Description", "File Transfer")
	c.Header("Content-Transfer-Encoding", "binary")
	c.Header("Content-Disposition", fmt.Sprintf("attachment; filename=%s", filename))
	c.Header("Content-Type", "application/octet-stream")

	// 鍙戦€佹枃浠?	c.File(filePath)
}
