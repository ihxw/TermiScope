...
func main() {
	...
	agentTransferRateLimiter := middleware.NewRateLimiter(600, 1*time.Minute)
	agentRelayRateLimiter := middleware.NewRateLimiter(12000, 1*time.Minute)
	...
	router.POST("/api/monitor/agent-transfer/report", agentTransferRateLimiter.RateLimitMiddlewareByKey(agentRateLimitKey), monitorHandler.ReportAgentTransfer)
	router.POST("/api/monitor/agent-transfer/relay/:transferId/source", agentRelayRateLimiter.RateLimitMiddlewareByKey(agentRateLimitKey), monitorHandler.UploadAgentRelayChunk)
	router.GET("/api/monitor/agent-transfer/relay/:transferId/destination", agentRelayRateLimiter.RateLimitMiddlewareByKey(agentRateLimitKey), monitorHandler.DownloadAgentRelayChunk)
	...
	router.GET("/api/sftp/native-download/:hostId", sftpHandler.DownloadWithTicket)
	router.HEAD("/api/sftp/native-download/:hostId", sftpHandler.DownloadWithTicket)
	...
	protected.Use(middleware.AuthMiddleware(cfg.Security.JWTSecret, db))
	{
		...
		protected.GET("/system/client-settings", systemHandler.GetClientSettings)
		...
		protected.POST("/ssh-hosts/:id/reveal-credentials", sshHostHandler.RevealCredentials)
		protected.GET("/ssh-hosts/:id/connect-credentials", sshHostHandler.GetConnectCredentials)
		...
		protected.PUT("/ssh-hosts/:id/monitor/transfer-port", monitorHandler.SetAgentTransferPort)
		...
		networkGroup.Use(middleware.AdminMiddleware())
		...
		protected.POST("/sftp/download-ticket/:hostId", sftpHandler.CreateDownloadTicket)
		...
		protected.POST("/sftp/sizes/:hostId", sftpHandler.GetDirSizes)
		...
		adminGroup.Use(middleware.AdminMiddleware())
		{
			...
			{
				system.POST("/backup/create", systemHandler.Backup)
				// Keep the original endpoint for older web and mobile clients.
				...
				system.GET("/logs", systemHandler.GetServerLogs)
	...
	registerFrontendStaticRoutes(router, frontendDistDir)
...
func registerFrontendStaticRoutes(router *gin.Engine, distDir string) {
	router.Static("/assets", filepath.Join(distDir, "assets"))
	router.Static("/icons", filepath.Join(distDir, "icons"))
	router.StaticFile("/favicon.ico", filepath.Join(distDir, "favicon.ico")) // Keep for legacy if file added later
	router.StaticFile("/favicon.png", filepath.Join(distDir, "favicon.png"))
