...
func main() {
	...
	firewall.SetListenPort(cfg.Server.Port)
	firewall.LogStartupStatus()
	...
	// Monitor routes
	router.POST("/api/monitor/pulse", agentRateLimiter.RateLimitMiddleware(), monitorHandler.Pulse)
	router.POST("/api/monitor/agent-event", agentRateLimiter.RateLimitMiddleware(), monitorHandler.AgentEvent)
	router.GET("/api/monitor/agent-commands", agentRateLimiter.RateLimitMiddleware(), monitorHandler.GetAgentCommands)
	...
	protected.Use(middleware.AuthMiddleware(cfg.Security.JWTSecret, db))
	{
		...
		adminGroup.Use(middleware.AdminMiddleware())
		{
			...
			system := adminGroup.Group("/system")
			{
				...
				system.GET("/firewall/status", systemHandler.GetFirewallStatus)
				system.POST("/firewall/initialize", systemHandler.InitializeFirewall)
				system.GET("/firewall/rules", systemHandler.GetFirewallRules)
				system.POST("/firewall/rules", systemHandler.AddFirewallRule)
				system.PUT("/firewall/rules/:number", systemHandler.UpdateFirewallRule)
				system.DELETE("/firewall/rules/:number", systemHandler.DeleteFirewallRule)
				system.GET("/firewall/kvm-compat", systemHandler.GetFirewallKVMCompatibility)
				system.POST("/firewall/kvm-compat/apply", systemHandler.ApplyFirewallKVMCompatibility)
				system.GET("/firewall/external-ports", systemHandler.GetFirewallExternalPorts)
				system.POST("/firewall/enable", systemHandler.EnableFirewall)
				system.POST("/firewall/disable", systemHandler.DisableFirewall)
				system.GET("/firewall/port-forwards", systemHandler.GetFirewallPortForwards)
				system.POST("/firewall/port-forwards", systemHandler.AddFirewallPortForward)
				system.PUT("/firewall/port-forwards/:number", systemHandler.UpdateFirewallPortForward)
				system.DELETE("/firewall/port-forwards/:number", systemHandler.DeleteFirewallPortForward)
				system.GET("/firewall/port-forward/settings", systemHandler.GetFirewallPortForwardSettings)
				system.PUT("/firewall/port-forward/settings", systemHandler.UpdateFirewallPortForwardSettings)
			}
