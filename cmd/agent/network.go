...
const (
	networkPingProbeCount = 5
	networkPingTimeout    = 20 * time.Second
	networkTCPTimeout     = 10 * time.Second
...
func (nm *NetworkMonitor) StartSimple() {
	go func() {
		...
		taskRunning := make(map[uint]bool)
		taskDone := make(chan uint, networkTaskWorkers*2)

		for {
			select {
			case taskID := <-taskDone:
				delete(taskRunning, taskID)
...
func (nm *NetworkMonitor) flushResults() {
	...
	if len(nm.resultsBuf) > maxBufferedResults {
		nm.resultsBuf = nm.resultsBuf[len(nm.resultsBuf)-maxBufferedResults:]
	}
	nm.bufMu.Unlock()
}
...
func ping(target string) (success bool, latency float64, packetLoss float64) {
	...
	pinger.Count = networkPingProbeCount
	pinger.Timeout = networkPingTimeout
	...
	latency = durationMilliseconds(stats.AvgRtt)
...
func tcping(target string, port int) (success bool, latency float64) {
	...
	start := time.Now()
	conn, err := net.DialTimeout("tcp", address, networkTCPTimeout)
	if err != nil {
		return false, -1
	}
	defer conn.Close()

	latency = durationMilliseconds(time.Since(start))
	return true, latency
}
