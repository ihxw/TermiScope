...
func executeAgentRelaySource(reportClient *http.Client, command agenttransfer.Command) {
	...
	if err == nil {
		defer source.Close()
		err = uploadAgentRelayChunksWithContext(ctx, reportClient, command, source)
	}
	if err != nil {
...
func uploadAgentRelayChunks(client *http.Client, command agenttransfer.Command, source *localAgentTransferSource) error {
	return uploadAgentRelayChunksWithContext(context.Background(), client, command, source)
}

func uploadAgentRelayChunksWithContext(ctx context.Context, client *http.Client, command agenttransfer.Command, source *localAgentTransferSource) error {
	...
	for {
		...
		if n > 0 {
			...
			if err := sendAgentRelayChunk(ctx, client, command, sequence, chunk, false, "", source); err != nil {
				return err
			}
			sequence++
		}
	...
	return sendAgentRelayChunk(ctx, client, command, sequence, nil, true, hex.EncodeToString(digest.Sum(nil)), source)
}
...
func agentTransferLongClient(base *http.Client) *http.Client {
	return &http.Client{Transport: base.Transport, CheckRedirect: base.CheckRedirect, Timeout: 2 * time.Minute}
}
