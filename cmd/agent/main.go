...
func main() {
	...
	svcConfig := &service.Config{
		...
		Arguments:   []string{"-server", serverURL, "-secret", secret, "-id", fmt.Sprintf("%d", hostID), "-interval", normalizedMetricsInterval().String()},
	}
	...
	if serverURL == "" || secret == "" || hostID == 0 {
		...
		if len(*svcFlag) == 0 {
			...
			if service.Interactive() {
				log.Fatal("Usage: agent -server <url> -secret <secret> -id <host_id> [-interval 10s] [-service install|uninstall|start|stop]")
			}
