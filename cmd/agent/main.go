...
type MetricData struct {
	...
	AgentTransferRelay        bool            `json:"agent_transfer_relay"`
	AgentTransferConfigurable bool            `json:"agent_transfer_configurable"`
...
var (
	...
	configPath              string
	agentTransferPort       int
	agentTransferCertSHA256 string
...
const (
	...
	countryDetectionRetry  = 10 * time.Minute
	countryDetectionURL    = "https://ipinfo.io/country"
)
...
func (p *program) run() {
	...
	initSystemInfo()
	startAgentTransferServer(agentTransferPort)
	defer stopActiveAgentTransferServer()
	...
	go detectCountryUntilSuccess(p.exit)
	...
	} else if _, err := attemptAgentSelfUpdate(client); err != nil {
	...
	for {
		...
		case <-updateTicker.C:
			...
			if _, err := attemptAgentSelfUpdate(client); err != nil {
				logError("Agent self-update check failed: %v", err)
...
func main() {
	...
	flag.IntVar(&agentTransferPort, "transfer-port", agenttransfer.DefaultTransferPort, "Agent-to-agent HTTPS transfer port")
	...
	serviceArgs := []string{"-server", serverURL, "-secret", secret, "-id", fmt.Sprintf("%d", hostID), "-interval", normalizedMetricsInterval().String(), "-transfer-port", fmt.Sprintf("%d", agentTransferPort)}
	if configPath != "" {
...
func loadAgentConfig(path string) error {
	...
	if cfg.TransferPort > 0 {
		agentTransferPort = cfg.TransferPort
	}
	return nil
}

func persistAgentTransferPort(path string, port int) error {
	data, err := os.ReadFile(path)
	if err != nil {
		return fmt.Errorf("read config: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("encode transfer port: %w", err)
	}
	values["transfer_port"] = encodedPort
...
func collectMetrics(includeStatic bool) MetricData {
	...
	transferPort, transferFingerprint := activeAgentTransferSnapshot()
	data := MetricData{
		HostID:                    hostID,
		Timestamp:                 time.Now().Unix(),
		AgentTransferPort:         transferPort,
		AgentTransferCertSHA256:   transferFingerprint,
		AgentTransferRelay:        true,
		AgentTransferConfigurable: true,
...
func sendAgentEvent(client *http.Client, event string, message string) error {
	return sendAgentEventPayload(client, map[string]interface{}{
		"host_id": hostID,
		"event":   event,
		"message": message,
	})
...
func sendAgentPortEvent(client *http.Client, event string, message string, port int) error {
	return sendAgentEventPayload(client, map[string]interface{}{
		"host_id": hostID,
		"event":   event,
		"message": message,
...
func sendAgentEventPayload(client *http.Client, data map[string]interface{}) error {
	jsonData, err := json.Marshal(data)
	if err != nil {
		return err
	}
