...
const (
	...
	agentUpdateDownloadTimeout = 5 * time.Minute
...
func attemptAgentSelfUpdate(client *http.Client) (bool, error) {
	agentUpdateMu.Lock()
	defer agentUpdateMu.Unlock()

	manifest, err := fetchAgentUpdateManifest(client)
	if err != nil {
		_ = sendAgentEvent(client, "update_failed", "更新失败")
		return false, err
	}
	if manifest == nil {
		return false, nil
	...
	if err := downloadAndApplyAgentUpdate(client, manifest); err != nil {
		_ = sendAgentEvent(client, "update_failed", "更新失败")
		return true, err
	}
	return true, nil
...
func fetchAgentUpdateManifest(client *http.Client) (*agentUpdateManifest, error) {
	...
	if currentHash, err := currentExecutableSHA256(); err == nil && strings.EqualFold(currentHash, manifest.SHA256) {
		logError("Agent update skipped: current binary already matches manifest hash despite version mismatch current=%s latest=%s", Version, manifest.Version)
		return nil, nil
	} else if err != nil {
		logError("Agent current binary checksum check failed: %v", err)
	}

	return &manifest, nil
}

func newAgentUpdateDownloadClient(client *http.Client) *http.Client {
	downloadClient := *client
	downloadClient.Timeout = agentUpdateDownloadTimeout
...
func downloadAndApplyAgentUpdate(client *http.Client, manifest *agentUpdateManifest) error {
	...
	// 指标上报客户端使用较短超时；更新下载需要独立的长超时，避免慢链路
	// 或大量 Agent 并发更新时在下载完成前被统一截断。
	downloadClient := newAgentUpdateDownloadClient(client)
...
func shellCommandLineArgs() string {
	args := []string{
		...
		"-interval", shellQuote(normalizedMetricsInterval().String()),
	}
	if insecure {
...
func windowsCommandLineArgs() string {
	args := []string{
		...
		"-interval", windowsQuote(normalizedMetricsInterval().String()),
	}
	if insecure {
		args = append(args, "-insecure")
	}
