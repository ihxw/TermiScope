...
func CheckAccountBruteForce(db *gorm.DB, username string, window time.Duration, threshold int) bool {
	var count int64
	since := time.Now().Add(-window)
	db.Model(&SecurityEvent{}).
		Where("username = ? AND event_type = ? AND created_at >= ?", username, LoginFailed, since).
		Count(&count)
	return count >= int64(threshold)
}
