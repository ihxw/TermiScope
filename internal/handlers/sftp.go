...
func (h *SftpHandler) Transfer(c *gin.Context) {
	...
	if !needsRename {
		if _, success := h.tryDirectRsync(c, srcSSH, dstHost, dstPassword, dstPrivateKey, resolvedSrc, resolvedDest, "", isDir, isDstDir); success {
			if req.Type == "cut" {
		...
		if _, success := h.tryServerSCP3(c, userID, srcHost, dstHost, srcPassword, srcPrivateKey, dstPassword, dstPrivateKey, resolvedSrc, resolvedDest, "", isDir, isDstDir, totalSize); success {
			if req.Type == "cut" {
				if err := deleteRemotePathViaSSH(srcSSH, resolvedSrc); err != nil {
					sendTransferEvent(c, map[string]interface{}{"type": "info", "message": "transfer ok but failed to remove source: " + err.Error()})
				}
