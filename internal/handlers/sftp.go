...
func sendTransferEvent(c *gin.Context, data map[string]interface{}) bool {
	var mu *sync.Mutex
	...
	if err := http.NewResponseController(c.Writer).SetWriteDeadline(time.Now().Add(10 * time.Second)); err != nil && !errors.Is(err, http.ErrNotSupported) {
		return false
	}
...
// Preferred: agent transfer between hosts, then rsync/scp fallbacks.
// Fallback: server-side SFTP relay when direct paths are unavailable.
func (h *SftpHandler) Transfer(c *gin.Context) {
	...
	var req struct {
		...
		Method       string `json:"method" binding:"omitempty,oneof=agent auto relay"`
	}
	...
	defer dstSftp.Close()
	defer dstSSH.Close()
	// Reload both peers before planning the transfer so direct mode uses the
	// latest heartbeat-advertised ports, capabilities, certificates, and TOFU
	// fingerprints from the server.
	if err := h.db.Where("id = ? AND user_id = ?", req.SourceHostID, userID).First(&srcHost).Error; err != nil {
	...
	transferErr := executeRemoteTransfer(transferRequiresRelay(req.Method, isSymlink, totalSize), remoteTransferAdapters{
		agentDirect: func() bool {
			_, success := h.tryAgentTransfer(c, srcHost, dstHost, resolvedSrc, stagedPath, isDir, totalSize)
			if success {
				transferMethod = "agent"
			}
			return success
		},
		agentRelay: func() bool {
			_ = h.deleteRecursive(dstSftp, stagedPath)
			_, success := h.tryAgentRelay(c, srcHost, dstHost, resolvedSrc, stagedPath, isDir, totalSize)
			if success {
				transferMethod = "agent-relay"
			}
			return success
		},
	...
	sendTransferEvent(c, map[string]interface{}{
		"type": "complete", "method": "atomic", "transport": transferMethod, "dest_path": targetPath,
	})
}
