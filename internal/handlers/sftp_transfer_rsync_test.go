...
func TestRsyncRemoteDest(t *testing.T) {
	got := rsyncRemoteDest("root", "10.0.0.2", "/data/backup", "", true, true)
	want := "root@10.0.0.2:/data/backup/"
	if got != want {
		t.Fatalf("got %q want %q", got, want)
	}

	got = rsyncRemoteDest("root", "10.0.0.2", "/data/backup", "copy.txt", false, true)
	want = "root@10.0.0.2:/data/backup/copy.txt"
	if got != want {
		t.Fatalf("got %q want %q", got, want)
	}

	got = rsyncRemoteDest("root", "10.0.0.2", "/data/backup/a.bin", "", false, false)
	want = "root@10.0.0.2:/data/backup/a.bin"
	if got != want {
		t.Fatalf("got %q want %q", got, want)
	}
}
...
func TestBuildRsyncCommand(t *testing.T) {
	...
	cmd := buildRsyncCommand(auth, dst, "/var/log/app.log", "/backup", "", false, true)
	...
	fileCmd := buildRsyncCommand(auth, dst, "/var/log/app.log", "/backup/app.log", "", false, false)
	if !strings.Contains(fileCmd, "root@10.0.0.2:/backup/app.log") {
		t.Fatalf("missing file dest: %s", fileCmd)
	}
}
