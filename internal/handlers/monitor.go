...
const maxAgentCommandLongPollTimeout = 25 * time.Second

func resolvePulseCountryCode(configured, reported string) (string, bool) {
	if configured != "" {
		return configured, false
	}
	countryCode, err := normalizeCountryCode(reported)
...
func (h *MonitorHandler) serverURL(request *http.Request) (string, error) {
	if h.Config.Server.PublicBaseURL != "" {
		return h.Config.Server.PublicBaseURL, nil
	}
	scheme := "http"
	if request.TLS != nil {
		scheme = "https"
	}
	parsed, err := url.Parse(fmt.Sprintf("%s://%s", scheme, request.Host))
...
func shellDoubleQuotedText(value string) string {
...
func (h *MonitorHandler) checkAllHostsTrafficReset() {
	...
	for i := range hosts {
		unlock := h.lockHostPulse(hosts[i].ID)
		var current models.SSHHost
		if err := h.DB.First(&current, hosts[i].ID).Error; err != nil {
			unlock()
			log.Printf("Traffic Reset Scheduler: Failed to reload host %d: %v", hosts[i].ID, err)
			continue
		}
		cycleStart := getCycleStartDate(time.Now(), current.NetResetDay)
		log.Printf("Traffic Reset Scheduler: Host %d (%s) ResetDay=%d LastResetDate='%s' CycleStart='%s' MonthlyRx=%d MonthlyTx=%d",
			current.ID, current.Name, current.NetResetDay, current.NetLastResetDate, cycleStart,
			current.NetMonthlyRx, current.NetMonthlyTx)
		if h.checkAndResetTraffic(&current) {
			log.Printf("Traffic Reset Scheduler: Reset completed for Host %d (%s)", current.ID, current.Name)
		}
		unlock()
	}
}
...
func getCycleStartDate(now time.Time, resetDay int) string {
	...
	getEffectiveResetDay := func(y int, m time.Month) int {
		lastDay := time.Date(y, m+1, 0, 0, 0, 0, 0, location).Day()
		if resetDay > lastDay {
			return lastDay
		}
		return resetDay
	...
	if day >= effectiveResetDay {
		currentCycleStart = time.Date(year, month, effectiveResetDay, 0, 0, 0, 0, location)
	} else {
		...
		currentCycleStart = time.Date(prevYear, prevMonth, prevEffectiveResetDay, 0, 0, 0, 0, location)
	}
...
while true; do
  ...
  # Country detection (one-time via ipinfo.io)
  if [ -z "$COUNTRY_DETECTED" ]; then
    COUNTRY_DETECTED=1
    country_code=""
    if command -v curl &> /dev/null; then
      country_code=$(curl -s --connect-timeout 3 --max-time 5 https://ipinfo.io/country 2>/dev/null | tr -d '[:space:]')
...
func (h *MonitorHandler) Pulse(c *gin.Context) {
	...
	if !host.MonitorEnabled {
		...
		if err := h.DB.Model(&host).Update("monitor_enabled", true).Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Database error"})
			return
		}
	...
	if err := h.DB.Select(
		...
		"country_code",
		"agent_transfer_port", "agent_transfer_cert_sha256", "agent_transfer_relay", "agent_transfer_configurable",
		"agent_transfer_requested_port", "agent_transfer_port_error",
	...
	if monitor.IsStalePulse(&host, data) {
	...
	// Billing cycle reset must happen before advancing the pulse ledger.
	dbUpdated := false
	if h.checkAndResetTraffic(&host) {
		dbUpdated = true
	}
	transition := monitor.AdvancePulse(&host, data, time.Now())
	data = transition.Metric
	deltaRx, deltaTx := transition.DeltaRx, transition.DeltaTx
	dbUpdated = dbUpdated || transition.Dirty

	if transition.TrafficAlert {
		msg := fmt.Sprintf("Host '%s' (ID: %d) has used %d%% of its traffic limit.\nUsed: %s / %s",
			host.Name, host.ID, transition.TrafficPercent,
			utils.FormatBytes(transition.TrafficUsed), utils.FormatBytes(host.NetTrafficLimit))
		utils.SendNotification(h.DB, host, fmt.Sprintf("Traffic Warning: %s", host.Name), msg, h.Config.Security.EncryptionKey)
	}

	if transition.CameOnline {
		go func(hostID uint) {
			if err := h.DB.Create(&models.MonitorStatusLog{
				HostID:    hostID,
				Status:    "online",
				CreatedAt: time.Now(),
			}).Error; err != nil {
				utils.LogError("Failed to record online status for host %d: %v", hostID, err)
			}
		}(host.ID)

		if host.NotifyOfflineEnabled && !transition.SuppressBackOnlineNotice {
		...
		if transition.SuppressBackOnlineNotice {
	...
	if dbUpdated {
		updateFields := map[string]interface{}{
			...
			"agent_transfer_port":           host.AgentTransferPort,
			"agent_transfer_cert_sha256":    host.AgentTransferCertSHA256,
			"agent_transfer_relay":          host.AgentTransferRelay,
			"agent_transfer_configurable":   host.AgentTransferConfigurable,
			"agent_transfer_requested_port": host.AgentTransferRequestedPort,
			"agent_transfer_port_error":     host.AgentTransferPortError,
			"status":                        host.Status,
		...
		if countryCode, changed := resolvePulseCountryCode(host.CountryCode, data.CountryCode); changed {
		...
		if err := h.DB.Model(&models.SSHHost{}).Where("id = ?", host.ID).Updates(updateFields).Error; err != nil {
			utils.LogError("Failed to persist pulse for host %d: %v", host.ID, err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Database error"})
			return
		}
	}
	...
	data.CountryCode = host.CountryCode
...
func (h *MonitorHandler) AgentEvent(c *gin.Context) {
	...
	switch event.Event {
	case "transfer_port_error":
		h.DB.Model(&models.SSHHost{}).
			Where("id = ? AND agent_transfer_requested_port = ?", host.ID, event.Port).
			Updates(map[string]interface{}{
				"agent_transfer_requested_port": 0,
				"agent_transfer_port_error":     event.Message,
			})
	case "transfer_port_updated":
		h.DB.Model(&models.SSHHost{}).
			Where("id = ? AND agent_transfer_requested_port = ?", host.ID, event.Port).
			Update("agent_transfer_port_error", "")
	}
...
// SetAgentTransferPort queues a runtime listener change for an authenticated
// host. The actual port remains heartbeat-owned until the agent binds it.
func (h *MonitorHandler) SetAgentTransferPort(c *gin.Context) {
	host, ok := loadSSHHostForUser(h.DB, c.Param("id"), c)
	if !ok {
		denyHostAccess(c)
		return
	}
	if !host.AgentTransferConfigurable {
		c.JSON(http.StatusConflict, gin.H{"error": "agent update required before configuring the transfer port"})
		return
	}
	var req agenttransfer.ConfigurePortCommand
	if err := c.ShouldBindJSON(&req); err != nil || req.Port <= 0 || req.Port > 65535 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "transfer port must be between 1 and 65535"})
		return
	}
	...
	err = h.DB.Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("host_id = ? AND command = ? AND processed = ?", host.ID, agenttransfer.ConfigurePortCommandName, false).
			Delete(&models.AgentCommand{}).Error; err != nil {
			return err
		}
		if err := tx.Model(&models.SSHHost{}).Where("id = ?", host.ID).Updates(map[string]interface{}{
			"agent_transfer_requested_port": req.Port,
			"agent_transfer_port_error":     "",
		}).Error; err != nil {
			return err
		}
		return tx.Create(&models.AgentCommand{
			HostID: host.ID, Command: agenttransfer.ConfigurePortCommandName, Payload: string(payload),
		}).Error
	})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to queue transfer port command"})
		return
	}
	notifyAgentCommand(host.ID)
	c.JSON(http.StatusAccepted, gin.H{
		"success": true, "requested_port": req.Port, "current_port": host.AgentTransferPort,
	})
}
...
func (h *MonitorHandler) Deploy(c *gin.Context) {
	...
	if secret == "" {
		generatedSecret, generateErr := utils.GenerateMonitorSecret()
		if generateErr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to generate monitor secret"})
			return
		}
		secret = generatedSecret
		host.MonitorSecret = secret
		if err := h.DB.Save(&host).Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to save monitor secret"})
			return
		}
	}

	serverURL, err := h.serverURL(c.Request)
	if err != nil {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "host key fingerprint mismatch") && newFp != "" {
	...
	sessionFailed := func(err error) bool {
		if err == nil {
			return false
		}
...
`
	...
	agentConfig := gin.H{
		...
		"transfer_port": agenttransfer.DefaultTransferPort,
	}
	...
	if err := prepareMonitorRedeploy(h.DB, host.ID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to prepare monitor redeployment"})
		return
	}
...
func (h *MonitorHandler) Stop(c *gin.Context) {
	...
	session, err := client.NewSession()
	if err != nil {
		log.Printf("Monitor Stop: Failed to create SSH session: %v", err)
		c.JSON(http.StatusOK, gin.H{"message": "Monitoring disabled (Agent stop failed: SSH session error)"})
		return
	}
	defer session.Close()
...
func (h *MonitorHandler) deployToHost(host *models.SSHHost, insecure bool, request *http.Request) error {
	// Generate Secret
	secret, err := utils.GenerateMonitorSecret()
	if err != nil {
		return fmt.Errorf("生成监控密钥失败: %w", err)
	}

	host.MonitorSecret = secret
	if err := h.DB.Save(host).Error; err != nil {
		return fmt.Errorf("保存监控密钥失败: %w", err)
	}

	serverURL, err := h.serverURL(request)
	if err != nil {
		return err
	}
...
`
	...
	if err := prepareMonitorRedeploy(h.DB, host.ID); err != nil {
		return fmt.Errorf("重置 Agent 心跳状态失败: %v", err)
	}
...
func (h *MonitorHandler) BatchStop(c *gin.Context) {
	...
	sendResult := func(result BatchStopResult) {
		select {
...
func (h *MonitorHandler) stopMonitorOnHost(host *models.SSHHost) error {
	// Update DB first
	if err := h.DB.Model(host).Update("monitor_enabled", false).Error; err != nil {
	...
	if err := session.Run(cmd); err != nil {
		return fmt.Errorf("停止监控服务失败: %w", err)
	}
	return nil
}
...
func (h *MonitorHandler) GetInstallScript(c *gin.Context) {
	...
	if host.MonitorSecret == "" {
		host.MonitorSecret, err = utils.GenerateMonitorSecret()
		if err != nil {
			c.String(http.StatusInternalServerError, "#!/bin/bash\necho 'Error: Failed to generate monitor secret'\nexit 1")
			return
		}
		if err := h.DB.Save(&host).Error; err != nil {
			c.String(http.StatusInternalServerError, "#!/bin/bash\necho 'Error: Failed to save monitor secret'\nexit 1")
			return
		}
		secret = host.MonitorSecret
	}
	...
	serverURL, err := h.serverURL(c.Request)
	if err != nil {
		c.String(http.StatusServiceUnavailable, "#!/bin/bash\necho 'Error: server public URL is not configured'\nexit 1")
		return
	}
	...
	script = strings.ReplaceAll(script, "{{HOST_NAME}}", shellDoubleQuotedText(host.Name))
	...
	if err := prepareMonitorRedeploy(h.DB, host.ID); err != nil {
		if osType == "windows" {
			c.String(http.StatusInternalServerError, "Write-Error 'Failed to prepare monitor redeployment'")
		} else {
			c.String(http.StatusInternalServerError, "#!/bin/bash\necho 'Error: Failed to prepare monitor redeployment'\nexit 1")
		}
		return
	}
...
func (h *MonitorHandler) GetAgentCommands(c *gin.Context) {
	...
	// Subscribe before querying so a command committed between the query and
	// the wait closes this channel and cannot be missed.
	wake := globalAgentCommandNotifier.waitChannel(host.ID)
	cmds, err := h.pendingAgentCommands(host.ID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to query commands"})
		return
	}
	waitSeconds, _ := strconv.Atoi(c.Query("wait"))
	if waitSeconds > int(maxAgentCommandLongPollTimeout/time.Second) {
		waitSeconds = int(maxAgentCommandLongPollTimeout / time.Second)
	}
	if len(cmds) == 0 && waitSeconds > 0 {
...
func (h *MonitorHandler) pendingAgentCommands(hostID uint) ([]models.AgentCommand, error) {
	var cmds []models.AgentCommand
	err := h.DB.Where("host_id = ? AND processed = ?", hostID, false).Find(&cmds).Error
	return cmds, err
}
