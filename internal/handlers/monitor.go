...
func (h *MonitorHandler) Stream(c *gin.Context) {
	userID, role, ok := authenticateMonitorStream(c, h.Config.Security.JWTSecret)
	if !ok {
...
`
	...
	execCmd := fmt.Sprintf("%s -server \"%s\" -secret \"%s\" -id %d -interval 10s", remoteBinaryPath, serverURL, secret, host.ID)
	if req.Insecure {
		execCmd += " -insecure"
	}
...
func (h *MonitorHandler) GetInstallScript(c *gin.Context) {
	...
	if hostID == "" || secret == "" {
		if osType == "windows" {
			c.String(http.StatusBadRequest, "Write-Error 'host_id and secret (query or Authorization Bearer) are required'")
		} else {
			c.String(http.StatusBadRequest, "#!/bin/bash\necho 'Error: host_id and secret required (use ?secret= or Authorization: Bearer)'\nexit 1")
		}
		return
	}
