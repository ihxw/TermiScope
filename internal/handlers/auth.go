...
var (
	...
	dummyPasswordHash, _ = bcrypt.GenerateFromPassword([]byte("termiscope-dummy-password"), bcrypt.DefaultCost)
)
...
func setAccessTokenCookie(c *gin.Context, token string, maxAge int) {
	setAuthCookie(c, "access_token", token, maxAge)
	setCSRFCookie(c, token != "" && maxAge > 0, maxAge)
}

func setRefreshTokenCookie(c *gin.Context, token string, maxAge int) {
	setAuthCookie(c, "refresh_token", token, maxAge)
}

func setAuthCookie(c *gin.Context, name, token string, maxAge int) {
...
func setCSRFCookie(c *gin.Context, enabled bool, maxAge int) {
	value := ""
	age := -1
	if enabled {
...
func tokenClaims(token, secret string) (*utils.Claims, *time.Time) {
...
func (h *AuthHandler) recordLoginHistory(c *gin.Context, user *models.User, accessToken, refreshToken string) LoginInfo {
	...
	if claims, _ := tokenClaims(accessToken, h.config.Security.JWTSecret); claims != nil {
		info.JTI = claims.ID
	}
	if claims, expiresAt := tokenClaims(refreshToken, h.config.Security.JWTSecret); claims != nil {
...
var errPasswordResetTokenUnavailable = errors.New("password reset token is unavailable")

// revokeUserSessions invalidates every access and refresh token currently tracked for a user.
// Callers must include password changes in the same transaction so a new password never leaves
// an old session usable.
func (h *AuthHandler) revokeUserSessions(tx *gorm.DB, userID uint) error {
	...
	for _, session := range sessions {
		...
		for _, jti := range []string{session.JTI, session.RefreshTokenJTI} {
			if jti == "" {
				continue
			}
			revoked := models.RevokedToken{JTI: jti, UserID: userID, ExpiresAt: expiresAt}
			if err := tx.Where("jti = ?", jti).FirstOrCreate(&revoked).Error; err != nil {
				return err
			}
		}
	}
...
func (h *AuthHandler) generateTokens(user *models.User) (string, string, error) {
	...
	accessToken, err := utils.GenerateTokenWithVersion(user.ID, user.Username, user.Role, "access", user.TokenVersion, accessExp, h.config.Security.JWTSecret)
	if err != nil {
		return "", "", err
	}

	refreshToken, err := utils.GenerateTokenWithVersion(user.ID, user.Username, user.Role, "refresh", user.TokenVersion, refreshExp, h.config.Security.JWTSecret)
	if err != nil {
		return "", "", err
	}
...
func (h *AuthHandler) Login(c *gin.Context) {
	...
	if models.CheckAccountBruteForce(h.db, user.Username, 15*time.Minute, 10) {
		models.SecurityEventLog(h.db, models.BruteForceDetected, models.SeverityHigh,
			user.ID, user.Username, c.ClientIP(), c.Request.UserAgent(), "Account login rate limit exceeded", nil)
		utils.ErrorResponse(c, http.StatusTooManyRequests, "too many attempts, please try again later")
		return
	}
	...
	if user.TwoFactorEnabled {
		...
		tempToken, err := utils.GenerateTokenWithVersion(user.ID, user.Username, user.Role, "2fa_temp", user.TokenVersion, 5*time.Minute, h.config.Security.JWTSecret)
		if err != nil {
	...
	refreshDuration, _ := time.ParseDuration(h.config.Security.RefreshExpiration)
	if refreshDuration == 0 {
		refreshDuration = 168 * time.Hour
	}
	setRefreshTokenCookie(c, refreshToken, int(refreshDuration.Seconds()))
...
func (h *AuthHandler) RefreshToken(c *gin.Context) {
	...
	claims, err := utils.ValidateToken(refreshToken, h.config.Security.JWTSecret)
	if err != nil {
		// Log detailed error for diagnostics
		utils.LogError("Refresh token validation failed: %v | IP: %s", err, c.ClientIP())
	...
	if claims.TokenVersion != user.TokenVersion {
		utils.ErrorResponse(c, http.StatusUnauthorized, "token invalidated")
		return
	}
	...
	newRefreshToken, err := utils.GenerateTokenWithVersion(user.ID, user.Username, user.Role, "refresh", user.TokenVersion, refreshExp, h.config.Security.JWTSecret)
	if err != nil {
	...
	newAccessClaims, newAccessExpiresAt := tokenClaims(accessToken, h.config.Security.JWTSecret)
	newRefreshClaims, newRefreshExpiresAt := tokenClaims(newRefreshToken, h.config.Security.JWTSecret)
	if newAccessClaims == nil || newRefreshClaims == nil {
		utils.LogError("Failed to parse refreshed token claims for user %d", user.ID)
	...
	if err := h.db.Transaction(func(tx *gorm.DB) error {
		if claims.ID != "" {
			updates := map[string]interface{}{
				"jti":               newAccessClaims.ID,
				"refresh_token_jti": newRefreshClaims.ID,
			}
			if newRefreshExpiresAt != nil {
				updates["expires_at"] = *newRefreshExpiresAt
			} else if newAccessExpiresAt != nil {
				updates["expires_at"] = *newAccessExpiresAt
			}

			if err := tx.Model(&models.LoginHistory{}).
				Where("refresh_token_jti = ? AND user_id = ?", claims.ID, claims.UserID).
				Updates(updates).Error; err != nil {
				return err
			}

			if err := tx.Create(&models.RevokedToken{
				JTI:       claims.ID,
		...
		utils.LogError("Failed to rotate refresh session for user %d: %v", user.ID, err)
		utils.ErrorResponse(c, http.StatusInternalServerError, "failed to refresh session")
		return
	}
	...
	setRefreshTokenCookie(c, newRefreshToken, int(refreshExp.Seconds()))
...
func (h *AuthHandler) ChangePassword(c *gin.Context) {
	...
	if err := h.db.Transaction(func(tx *gorm.DB) error {
		...
		return h.revokeUserSessions(tx, user.ID)
	}); err != nil {
...
func (h *AuthHandler) ForgotPassword(c *gin.Context) {
	...
	if err := h.db.Where("email = ?", req.Email).First(&user).Error; err != nil {
		...
		log.Printf("ForgotPassword: request for unknown account (IP: %s)", c.ClientIP())
		utils.SuccessResponse(c, http.StatusOK, gin.H{
	...
	origin := h.config.Server.PublicBaseURL
	if origin == "" {
		utils.ErrorResponse(c, http.StatusServiceUnavailable, "未配置公开访问地址，请联系管理员设置 public_base_url")
		return
	}
...
func (h *AuthHandler) ResetPassword(c *gin.Context) {
	...
	if err := h.db.Transaction(func(tx *gorm.DB) error {
		result := tx.Model(&models.PasswordResetToken{}).
			Where("id = ? AND used = ? AND expires_at > ?", resetToken.ID, false, time.Now()).
			Update("used", true)
		if result.Error != nil {
			return result.Error
		}
		if result.RowsAffected != 1 {
			return errPasswordResetTokenUnavailable
		}
		if err := tx.Model(&models.PasswordResetToken{}).Where("user_id = ?", user.ID).Update("used", true).Error; err != nil {
			return err
		}
		if err := tx.Save(&user).Error; err != nil {
			return err
		...
		if errors.Is(err, errPasswordResetTokenUnavailable) {
			utils.ErrorResponse(c, http.StatusBadRequest, "Invalid or expired token")
		} else {
