...
func (h *NetworkMonitorHandler) UpdateTask(c *gin.Context) {
	...
	if req.Frequency != nil {
		if *req.Frequency <= 0 {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Frequency must be positive"})
			return
		}
...
func (h *NetworkMonitorHandler) DeleteTask(c *gin.Context) {
	...
	if err := h.DB.Delete(&models.NetworkMonitorTask{}, task.ID).Error; err != nil {
...
func (h *NetworkMonitorHandler) GetTaskStats(c *gin.Context) {
	...
	parsedTaskID, err := strconv.ParseUint(taskID, 10, 64)
...
// --- Template Endpoints ---
func requireNetworkTemplateAdmin(c *gin.Context) bool {
...
func (h *NetworkMonitorHandler) CreateTemplate(c *gin.Context) {
	if !requireNetworkTemplateAdmin(c) {
		return
	}
...
func (h *NetworkMonitorHandler) UpdateTemplate(c *gin.Context) {
	...
	id := c.Param("id")
	tmplID, err := strconv.ParseUint(id, 10, 64)
	if err != nil || tmplID == 0 {
	...
	var tmpl models.NetworkMonitorTemplate
	if err := h.DB.First(&tmpl, uint(tmplID)).Error; err != nil {
...
func (h *NetworkMonitorHandler) DeleteTemplate(c *gin.Context) {
	...
	if err := tx.Model(&models.NetworkMonitorTask{}).Where("template_id = ?", uint(tmplID)).Update("template_id", 0).Error; err != nil {
		tx.Rollback()
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to unlink template tasks"})
		return
	}
	if err := tx.Delete(&models.NetworkMonitorTemplate{}, uint(tmplID)).Error; err != nil {
		tx.Rollback()
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete template"})
		return
	}
...
func (h *NetworkMonitorHandler) GetTemplateAssignments(c *gin.Context) {
	...
	tmplID := c.Param("id")
	parsedID, err := strconv.ParseUint(tmplID, 10, 64)
	if err != nil || parsedID == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid template id"})
		return
	}
	var tmpl models.NetworkMonitorTemplate
	if err := h.DB.First(&tmpl, uint(parsedID)).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Template not found"})
		return
	}
