...
func appendServerSSHHost(cfg *strings.Builder, alias string, host models.SSHHost, password, privateKey, dir string) error {
	...
	fmt.Fprintf(cfg, "Host %s\n  HostName %s\n  Port %d\n  User %s\n", alias, host.Host, port, host.Username)
...
func serverSCPEndpoints(resolvedSrc, resolvedDest, destFileName string, isDir bool) (src, dst string) {
	src = "ts-src:" + resolvedSrc
	dst = "ts-dst:" + scpDestPath(resolvedDest, destFileName, isDir)
	return src, dst
}
...
func (h *SftpHandler) tryServerSCP3(
	...
	srcArg, dstArg := serverSCPEndpoints(resolvedSrc, resolvedDest, destFileName, isDir)
	args := []string{
		"-3",
		"-F", bundle.configPath,
		"-o", "BatchMode=yes",
	...
	if err != nil {
		...
		sendTransferEvent(c, map[string]interface{}{"type": "error", "message": "server scp failed: " + msg})
		return true, false
	}
