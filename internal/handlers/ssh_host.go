...
// validateSSHConnectionFields rejects values that cannot be represented safely
// in either an SSH connection or generated OpenSSH configuration.
func validateSSHConnectionFields(host, username string) error {
	for field, value := range map[string]string{"host": host, "username": username} {
		if value == "" {
			continue
		}
		for _, r := range value {
...
func normalizeCountryCode(code string) (string, error) {
	...
	if len(code) != 2 || code[0] < 'A' || code[0] > 'Z' || code[1] < 'A' || code[1] > 'Z' {
		return "", errors.New("country_code must be a two-letter ISO country code")
	}
	return code, nil
}
...
type CreateSSHHostRequest struct {
	...
	RemoteShell string `json:"remote_shell" binding:"omitempty,oneof=default powershell pwsh cmd bash"`
	OsType      string `json:"os_type" binding:"omitempty,oneof=linux darwin windows"`
...
func (h *SSHHostHandler) List(c *gin.Context) {
	...
	for i := range hosts {
		hosts[i].MonitorSecret = ""
		hosts[i].AgentTransferAvailable = agentTransferReady(hosts[i], time.Now())
	}
...
func (h *SSHHostHandler) Get(c *gin.Context) {
	...
	host.AgentTransferAvailable = agentTransferReady(host, time.Now())

	utils.SuccessResponse(c, http.StatusOK, host)
}

// RevealCredentials verifies the current login password before returning credentials.
func (h *SSHHostHandler) RevealCredentials(c *gin.Context) {
	userID := middleware.GetUserID(c)
	var req struct {
		CurrentPassword string `json:"current_password" binding:"required"`
	}
	if err := c.ShouldBindJSON(&req); err != nil {
		utils.ErrorResponse(c, http.StatusBadRequest, "current password required")
		return
	}

	var user models.User
	if err := h.db.First(&user, userID).Error; err != nil || !user.CheckPassword(req.CurrentPassword) {
		utils.ErrorResponse(c, http.StatusUnauthorized, "incorrect current password")
		return
	}

	var host models.SSHHost
	if err := h.db.Where("id = ? AND user_id = ?", c.Param("id"), userID).First(&host).Error; err != nil {
		utils.ErrorResponse(c, http.StatusNotFound, "host not found")
		return
	}
...
// GetConnectCredentials returns decrypted connection credentials for the
// authenticated owner so clients can open a direct local-network SSH session.
// @Summary Get SSH Host Connect Credentials
// @Description Get decrypted connection info of a host for direct client-side SSH
// @Tags Host
// @Security BearerAuth
// @Produce json
// @Param id path int true "Host ID"
...
// @Failure 404 {object} map[string]string "Host not found"
// @Router /ssh-hosts/{id}/connect-credentials [get]
func (h *SSHHostHandler) GetConnectCredentials(c *gin.Context) {
	userID := middleware.GetUserID(c)
	id := c.Param("id")
	...
	if host.HostType == "monitor_only" {
		utils.ErrorResponse(c, http.StatusBadRequest, "host does not support SSH connections")
		return
	}

	password, privateKey := h.decryptCredentials(&host)
	utils.SuccessResponse(c, http.StatusOK, gin.H{
		"id":          host.ID,
		"name":        host.Name,
		"host":        host.Host,
...
func (h *SSHHostHandler) Create(c *gin.Context) {
	...
	if err := validateSSHConnectionFields(req.Host, req.Username); err != nil {
	...
	monitorSecret, err := utils.GenerateMonitorSecret()
	if err != nil {
		utils.ErrorResponse(c, http.StatusInternalServerError, "failed to generate monitor secret")
		return
	}
	host.MonitorSecret = monitorSecret
...
func (h *SSHHostHandler) Update(c *gin.Context) {
	...
	if req.CountryCode != nil {
		normalized, err := normalizeCountryCode(*req.CountryCode)
		if err != nil {
			utils.ErrorResponse(c, http.StatusBadRequest, err.Error())
			return
		}
	...
	if req.RemoteShell != nil {
		host.RemoteShell = *req.RemoteShell
	}
	if req.OsType != nil {
	...
	if req.ResetTraffic || req.NetTrafficLimit > 0 || req.NetTrafficUsedAdjustment > 0 || req.NetTrafficCounterMode != "" {
		...
		if req.NetTrafficCounterMode != "" {
			host.NetTrafficCounterMode = monitor.NormalizeTrafficCounterMode(req.NetTrafficCounterMode)
		}
	}
	...
	fields := []string{
		...
		"ExpirationDate", "BillingPeriod", "BillingAmount", "Currency", "Flag", "CountryCode",
	}
	if netInterfaceChanged {
		fields = append(fields, "NetLastRawRx", "NetLastRawTx")
	}

	if req.ResetTraffic {
		fields = append(fields, "NetMonthlyRx", "NetMonthlyTx", "NetLastRawRx", "NetLastRawTx", "NetLastResetDate", "TrafficAlerted")
...
func (h *SSHHostHandler) Delete(c *gin.Context) {
	...
	invalidateAgentHostCache(host.MonitorSecret)
	invalidatePulseHostCache(uint(id))
...
func (h *SSHHostHandler) PermanentDelete(c *gin.Context) {
	...
	var host models.SSHHost
	if err := h.db.Unscoped().Where("id = ? AND user_id = ?", id, userID).First(&host).Error; err != nil {
		utils.ErrorResponse(c, http.StatusNotFound, "host not found")
		return
	}
	...
	if err := h.db.Transaction(func(tx *gorm.DB) error {
		var taskIDs []uint
		if err := tx.Model(&models.NetworkMonitorTask{}).Where("host_id = ?", id).Pluck("id", &taskIDs).Error; err != nil {
			return err
		}
		if len(taskIDs) > 0 {
			if err := tx.Where("task_id IN ?", taskIDs).Delete(&models.NetworkMonitorResult{}).Error; err != nil {
				return err
			}
		}
		if err := tx.Where("host_id = ?", id).Delete(&models.NetworkMonitorTask{}).Error; err != nil {
			return err
		}
		result := tx.Unscoped().Where("id = ? AND user_id = ?", id, userID).Delete(&models.SSHHost{})
		if result.Error != nil {
			return result.Error
		}
		if result.RowsAffected == 0 {
...
func (h *SSHHostHandler) TestConnection(c *gin.Context) {
	...
	target := net.JoinHostPort(host.Host, strconv.Itoa(port))
	start := time.Now()
...
func (r *ReorderRequest) UnmarshalJSON(data []byte) error {
	type Alias ReorderRequest
	var temp struct {
		DeviceIds      []uint `json:"DeviceIds"`
		DeviceIdsSnake []uint `json:"device_ids"`
		*Alias
	}
	temp.Alias = (*Alias)(r)
	...
	if len(temp.DeviceIds) > 0 {
		r.DeviceIds = temp.DeviceIds
	} else if len(temp.DeviceIdsSnake) > 0 {
		r.DeviceIds = temp.DeviceIdsSnake
	}
	return nil
}
...
func (h *SSHHostHandler) Reorder(c *gin.Context) {
	...
	if len(req.DeviceIds) == 0 {
		utils.ErrorResponse(c, http.StatusBadRequest, "device_ids cannot be empty")
		return
	}

	// Validate that all requested DeviceIds exist and belong to the authenticated userID
	uniqueMap := make(map[uint]bool)
	var uniqueReqIDs []uint
	for _, id := range req.DeviceIds {
		if uniqueMap[id] {
			utils.ErrorResponse(c, http.StatusBadRequest, "device_ids cannot contain duplicates")
			return
		}
		uniqueMap[id] = true
	...
	var userHostIDs []uint
	if err := h.db.Model(&models.SSHHost{}).Where("user_id = ?", userID).Pluck("id", &userHostIDs).Error; err != nil {
		errWrapped := fmt.Errorf("failed to retrieve user hosts: %w", err)
		utils.ErrorResponse(c, http.StatusInternalServerError, errWrapped.Error())
		return
	}

	userHostMap := make(map[uint]bool)
	for _, id := range userHostIDs {
		userHostMap[id] = true
	}

	for _, id := range uniqueReqIDs {
		if !userHostMap[id] {
			errForbidden := errors.New("invalid or unauthorized host ID")
			utils.ErrorResponse(c, http.StatusForbidden, errForbidden.Error())
			return
		}
	}
