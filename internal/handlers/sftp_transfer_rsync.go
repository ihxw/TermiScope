...
if [ "$SCANNED_FP" != '%s' ]; then
	...
	%s
	exit 1
fi`, escapedHostsPath, dstHost.Port, utils.ShellEscape(dstHost.Host), expectedFpStr, expectedFpStr, cleanupCmd)
	}
	return fmt.Sprintf("export TMP_HOSTS=%s\nssh-keyscan -p %d %s > $TMP_HOSTS 2>/dev/null", escapedHostsPath, dstHost.Port, utils.ShellEscape(dstHost.Host))
}
