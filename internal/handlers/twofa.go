...
type pendingTwoFASecret struct {
	Secret   string
	Expires  time.Time
	Failures int
}

var pendingTwoFASecrets sync.Map
...
func (h *TwoFactorHandler) Setup2FA(c *gin.Context) {
	...
	pendingTwoFASecrets.Store(userID, pendingTwoFASecret{
		Secret:  key.Secret(),
...
func (h *TwoFactorHandler) VerifySetup2FA(c *gin.Context) {
	...
	pendingValue, ok := pendingTwoFASecrets.Load(userID)
	if !ok {
		utils.ErrorResponse(c, http.StatusBadRequest, "2FA secret not found")
		return
	}
	pending := pendingValue.(pendingTwoFASecret)
	if time.Now().After(pending.Expires) {
		pendingTwoFASecrets.Delete(userID)
		utils.ErrorResponse(c, http.StatusBadRequest, "2FA secret expired")
		return
	}
	if pending.Failures >= 5 {
		utils.ErrorResponse(c, http.StatusTooManyRequests, "too many verification attempts")
		return
	}
	...
	if !valid {
		pending.Failures++
		pendingTwoFASecrets.Store(userID, pending)
