...
func (h *SSHWebSocketHandler) HandleWebSocket(c *gin.Context) {
	...
	// Gorilla handles Pong control frames inside ReadMessage instead of returning
	// them as messages. Refresh the deadline here so an otherwise idle terminal
	// remains attached while the browser is still responding to keepalives.
	if idleTimeout > 0 {
	...
	if record {
		...
		fileName := fmt.Sprintf("%d-%d-%d-%s.cast", userID, host.ID, time.Now().UnixNano(), utils.GenerateRandomString(8))
