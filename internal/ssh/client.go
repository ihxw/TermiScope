...
type SSHConfig struct {
	...
	// InsecureSkipVerify disables host key verification (batch deploy "insecure" option only).
...
// Shell starts an interactive shell using the SSH server's default shell.
...
// StartInteractive starts an interactive session using remoteShell and remote os_type.
func (c *SSHClient) StartInteractive(remoteShell, osType string) error {
	command, useDefault := ResolveRemoteShell(remoteShell, osType)
	if useDefault {
		return c.Shell()
	}
	if c.session == nil {
