...
const maxUpdateDownloadBytes = 512 * 1024 * 1024

var latestReleaseURL = "https://api.github.com/repos/ihxw/TermiScope/releases/latest"
...
func CheckForUpdate(currentVersion string) (*UpdateInfo, error) {
	return checkForUpdate(currentVersion, false)
}

// CheckForTrustedUpdate checks if a newer version is available and installable.
func CheckForTrustedUpdate(currentVersion string) (*UpdateInfo, error) {
	return checkForUpdate(currentVersion, true)
}

func checkForUpdate(currentVersion string, requireChecksum bool) (*UpdateInfo, error) {
	...
	sha256Value, err := fetchAssetSHA256(client, release.Assets, bestAsset.Name)
	if err != nil {
		if requireChecksum {
			return nil, err
		}
		updateInfo.ChecksumError = err.Error()
		return updateInfo, nil
	}

	updateInfo.SHA256 = sha256Value
	updateInfo.Trusted = true
	return updateInfo, nil
}

// PerformUpdate downloads, extracts (if needed), replaces, and restarts
func PerformUpdate(downloadURL, expectedSHA256 string, statusCallback func(string)) error {
	...
	if len(expectedSHA256) != 64 {
		return fmt.Errorf("trusted update checksum is required")
	}
	// 1. Download
	...
	hasher := sha256.New()
	_, err = io.Copy(tmpFile, io.TeeReader(io.LimitReader(resp.Body, maxUpdateDownloadBytes+1), hasher))
	tmpFile.Close()
	if err != nil {
		return fmt.Errorf("failed to save download: %v", err)
	}
	if info, statErr := os.Stat(tmpFile.Name()); statErr != nil {
		return fmt.Errorf("failed to stat download: %v", statErr)
	} else if info.Size() > maxUpdateDownloadBytes {
		return fmt.Errorf("download too large")
	}
...
func validateReleaseAssetURL(rawURL string) error {
	...
	if u.Host != "github.com" && u.Host != "objects.githubusercontent.com" {
		return fmt.Errorf("untrusted update host")
	}
	if u.Host == "github.com" && !strings.HasPrefix(u.Path, "/ihxw/TermiScope/releases/download/") {
		return fmt.Errorf("untrusted update path")
	}
	return nil
}

func fetchAssetSHA256(client *http.Client, assets []Asset, assetName string) (string, error) {
	checksumName := strings.ToLower(assetName + ".sha256")
	var checksumAsset *Asset
	for _, asset := range assets {
		if strings.ToLower(asset.Name) == checksumName {
			checksumAsset = &asset
			break
		}
	}
	if checksumAsset == nil {
		return "", fmt.Errorf("trusted checksum asset %s is required", assetName+".sha256")
	}
	if err := validateReleaseAssetURL(checksumAsset.BrowserDownloadURL); err != nil {
		return "", err
	}
	...
	if _, err := hex.DecodeString(fields[0]); err != nil {
		return "", fmt.Errorf("invalid checksum value")
	}
