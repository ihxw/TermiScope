...
type SSHConfig struct {
	...
	SFTPConcurrency       int    `mapstructure:"sftp_concurrency"`
	MaxSFTPOperations     int    `mapstructure:"max_sftp_operations"`
	MaxSFTPOperationsUser int    `mapstructure:"max_sftp_operations_per_user"`
}
...
func LoadConfig() (*Config, error) {
	...
	viper.SetDefault("server.trusted_proxies", []string{}) // Empty = do not trust forwarded client IP headers
	...
	viper.SetDefault("ssh.sftp_concurrency", 32)
	viper.SetDefault("ssh.max_sftp_operations", 16)
	viper.SetDefault("ssh.max_sftp_operations_per_user", 8)
	...
	if resolvedConfigPath != "" {
		if err := os.Chmod(resolvedConfigPath, 0600); err != nil {
...
func (c *Config) SaveConfig() error {
	...
	viper.Set("server.trusted_proxies", c.Server.TrustedProxies)
	...
	viper.Set("ssh.sftp_concurrency", c.SSH.SFTPConcurrency)
	viper.Set("ssh.max_sftp_operations", c.SSH.MaxSFTPOperations)
	viper.Set("ssh.max_sftp_operations_per_user", c.SSH.MaxSFTPOperationsUser)
