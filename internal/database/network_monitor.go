...
// MaxNetworkMonitorChartPoints caps raw rows fetched before downsampling.
const MaxNetworkMonitorChartPoints = ChartDisplayMaxPoints + 500
...
func queryNetworkMonitorResults(db *gorm.DB, taskID uint, since time.Time) ([]models.NetworkMonitorResult, error) {
	...
	results = RollupNetworkResults(results, NetworkLatencyRollupSampleCount, networkLatencyRollupMaxGap(db, taskID))
	return DownsampleNetworkResults(results, ChartDisplayMaxPoints), nil
}

func networkLatencyRollupMaxGap(db *gorm.DB, taskID uint) time.Duration {
	const defaultFrequency = 60

	var task models.NetworkMonitorTask
	if err := db.Select("frequency").First(&task, taskID).Error; err != nil || task.Frequency <= 0 {
		return time.Duration(defaultFrequency*3) * time.Second
	}
	return time.Duration(task.Frequency*3) * time.Second
}
