...
func RunMigrations(db *gorm.DB) error {
	...
	db.Exec("CREATE INDEX IF NOT EXISTS idx_monitor_records_host_created ON monitor_records(host_id, created_at)")
	db.Exec("CREATE INDEX IF NOT EXISTS idx_ssh_hosts_monitor_secret ON ssh_hosts(monitor_secret)")
	db.Exec("CREATE INDEX IF NOT EXISTS idx_traffic_reset_host_date_status ON monitor_traffic_reset_logs(host_id, reset_date, status)")
	db.Exec("CREATE INDEX IF NOT EXISTS idx_sftp_path_bookmarks_lookup ON sftp_path_bookmarks(user_id, host_id, type, position)")
