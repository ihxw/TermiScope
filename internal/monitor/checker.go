...
func checkOfflineHosts(db *gorm.DB, encryptionKey string) {
	...
	for _, host := range onlineHosts {
		...
		if host.LastPulse.Before(threshold) {
			result := db.Model(&models.SSHHost{}).
				Where("id = ? AND monitor_enabled = ? AND status = ? AND last_pulse < ?", host.ID, true, "online", threshold).
			...
			if result.Error != nil {
				log.Printf("Monitor Checker: Failed to update host %d status: %v", host.ID, result.Error)
				continue
			}
	...
	for _, host := range pendingHosts {
		result := db.Model(&models.SSHHost{}).
			Where("id = ? AND monitor_enabled = ? AND status = ? AND offline_notified = ?", host.ID, true, "offline", false).
			Update("offline_notified", true)
		if result.Error != nil {
			log.Printf("Monitor Checker: Failed to mark host %d offline notification sent: %v", host.ID, result.Error)
			continue
		}
		if result.RowsAffected == 0 {
			continue
