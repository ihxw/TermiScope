...
func IsStalePulse(host *models.SSHHost, data MetricData) bool {
	if data.Timestamp <= 0 || host.LastAgentTimestamp <= 0 {
		return false
	}
	if data.Timestamp == host.LastAgentTimestamp {
		return true
	}
	return data.Timestamp+5 < host.LastAgentTimestamp
}
...
func AdvancePulse(host *models.SSHHost, data MetricData, now time.Time) PulseTransition {
	...
	if host.AgentTransferRequestedPort != 0 && host.AgentTransferRequestedPort == data.AgentTransferPort {
		host.AgentTransferRequestedPort = 0
		host.AgentTransferPortError = ""
		result.Dirty = true
	}
	...
	if host.AgentTransferConfigurable != data.AgentTransferConfigurable {
		host.AgentTransferConfigurable = data.AgentTransferConfigurable
		result.Dirty = true
	}
