...
const (
	...
	clientBroadcastMin   = 1 * time.Second
)
...
func (h *Hub) filterHostsForClient(client *wsClient, hosts []*MetricData) []*MetricData {
	...
	now := time.Now().Unix()
	for _, v := range hosts {
		if now-v.LastUpdated < 15 && h.clientMayViewHost(client, v.HostID) {
			out = append(out, v)
		}
	}
	return out
...
func (h *Hub) broadcastDirty() {
	...
	for _, id := range dirtyIDs {
		if v, ok := h.hosts[id]; ok && now-v.LastUpdated < 15 {
	...
	nowTime := time.Now()
	for client := range h.clients {
		if nowTime.Sub(client.lastBroadcastAt) < clientBroadcastMin {
			continue
		}
		...
		if err := client.conn.WriteMessage(websocket.TextMessage, jsonMsg); err != nil {
			...
			client.lastBroadcastAt = nowTime
		}
	}
	h.clientsMu.RUnlock()
