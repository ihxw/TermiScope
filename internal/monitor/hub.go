...
const (
	...
	agentUpdateStatusTimeout = 10 * time.Minute
)
...
type MetricData struct {
	...
	AgentTransferConfigurable bool    `json:"agent_transfer_configurable,omitempty"`
...
type Hub struct {
	...
	pendingAgentEvents map[uint]AgentEvent
	offlineHosts       map[uint]bool
	dirtyHosts         map[uint]bool
	agentUpdateStarted map[uint]time.Time
...
func NewHub() *Hub {
	return &Hub{
		...
		pendingAgentEvents: make(map[uint]AgentEvent),
		offlineHosts:       make(map[uint]bool),
		dirtyHosts:         make(map[uint]bool),
		agentUpdateStarted: make(map[uint]time.Time),
	}
}
...
func (h *Hub) Run() {
	...
	for {
		...
		case event := <-h.agentEventChan:
			h.runSafely(func() { h.applyAgentEvent(event) })
		...
		case <-staleTicker.C:
			h.runSafely(func() {
				...
				h.flushPendingAgentEvents()
				h.expireAgentUpdateStatuses(time.Now())
			})
		}
	}
}

func (h *Hub) runSafely(fn func()) {
	defer func() {
...
func (h *Hub) applyUpdate(data MetricData) {
	...
	if exists {
		...
		if reportedAgentVersion != "" && reportedAgentVersion != prev.AgentVersion && prev.AgentUpdateStatus != "" {
			data.AgentUpdateStatus = ""
			delete(h.agentUpdateStarted, data.HostID)
		}
...
func (h *Hub) flushPendingAgentEvents() {
	h.pendingMu.Lock()
	if len(h.pendingAgentEvents) == 0 {
		h.pendingMu.Unlock()
		return
	}
	pending := h.pendingAgentEvents
	h.pendingAgentEvents = make(map[uint]AgentEvent)
	h.pendingMu.Unlock()

	for _, event := range pending {
		h.applyAgentEvent(event)
	}
}

func (h *Hub) applyAgentEvent(event AgentEvent) {
	h.hostsMu.Lock()
	if host, ok := h.hosts[event.HostID]; ok {
		switch event.Event {
		case "command", "update_downloading", "update_installing":
			host.AgentUpdateStatus = event.Message
			h.agentUpdateStarted[event.HostID] = time.Now()
		case "update_failed", "update_idle", "update_complete":
			host.AgentUpdateStatus = ""
			delete(h.agentUpdateStarted, event.HostID)
		}
	}
	h.hostsMu.Unlock()
...
func (h *Hub) expireAgentUpdateStatuses(now time.Time) {
	var expired []uint
	h.hostsMu.Lock()
	for hostID, startedAt := range h.agentUpdateStarted {
		if now.Sub(startedAt) < agentUpdateStatusTimeout {
			continue
		}
		if host, ok := h.hosts[hostID]; ok {
			host.AgentUpdateStatus = ""
			expired = append(expired, hostID)
		}
		delete(h.agentUpdateStarted, hostID)
	}
	h.hostsMu.Unlock()
	for _, hostID := range expired {
		h.markDirty(hostID)
	}
}
...
func (h *Hub) evictStaleHosts() {
	...
	for id, v := range h.hosts {
		if now-v.LastUpdated > hostStaleSeconds {
			...
			delete(h.agentUpdateStarted, id)
		}
	}
	h.hostsMu.Unlock()
}
...
func (h *Hub) AgentEvent(event AgentEvent) {
	...
	default:
		h.pendingMu.Lock()
		h.pendingAgentEvents[event.HostID] = event
		h.pendingMu.Unlock()
	}
}
