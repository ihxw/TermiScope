...
func (m *netshManager) Status() (Status, error) {
	if !m.available() {
		return finalizeStatus(Status{
			...
			Capabilities: windowsFirewallCapabilities(),
		}), nil
	}
	...
	return finalizeStatus(Status{
		...
		Enabled:      enabled,
		Backend:      "netsh",
		Warning:      "Disabling uses Windows global firewall state for all profiles.",
		Capabilities: windowsFirewallCapabilities(),
	}), nil
}

func windowsFirewallCapabilities() FirewallCapabilities {
	return FirewallCapabilities{
		CanReject:               false,
		CanPortForwardUDP:       false,
		CanSourcePortForward:    false,
		GlobalDisable:           true,
		ListsSystemRules:        true,
		SupportsKVMCompat:       false,
		SupportsBootPersistence: false,
	}
}
...
func (m *netshManager) Rules() ([]Rule, error) {
	...
	flushRule := func() {
		...
		current.ID = current.Comment
		current.Managed = isTermiScopeNetshRule(current.Comment)
...
func isTermiScopeNetshRule(name string) bool {
	lower := strings.ToLower(strings.TrimSpace(name))
	return strings.HasPrefix(lower, "termiscope-")
}

func netshTermiScopeRuleName(action, protocol, port, comment string) string {
	parts := []string{"TermiScope", strings.ToLower(strings.TrimSpace(action))}
	...
	if port = strings.TrimSpace(port); port != "" {
		parts = append(parts, sanitizeNetshNamePart(port))
	}
	if comment = strings.TrimSpace(comment); comment != "" {
		parts = append(parts, sanitizeNetshNamePart(comment))
	}
	return strings.Join(parts, "-")
}

func sanitizeNetshNamePart(s string) string {
...
func (m *netshManager) UpdateRule(number int, req AddRuleRequest) error {
	if !m.available() {
		return fmt.Errorf("netsh is not available")
	}
	if number < 1 {
	...
	if !target.Managed {
		return fmt.Errorf("refusing to edit non-Termiscope Windows Firewall rule: %s", target.Comment)
	}
	if _, err := m.run("advfirewall", "firewall", "delete", "rule", "name="+target.Comment); err != nil {
		return err
	}
	if err := m.AddRule(req); err != nil {
		_ = m.addRuleOnce(ruleToAddRequest(*target))
		return err
	}
	return nil
}
...
func (m *netshManager) addRuleOnce(req AddRuleRequest) error {
	...
	ruleName := netshTermiScopeRuleName(action, protocol, port, comment)
...
func (m *netshManager) DeleteRule(number int) error {
	...
	if !target.Managed {
		return fmt.Errorf("refusing to delete non-Termiscope Windows Firewall rule: %s", target.Comment)
	}
...
func (m *netshManager) Enable(req EnableFirewallRequest) error {
	...
	for _, item := range mergeEnableAllowWithBaseline(req.Allow) {
...
func (m *netshManager) UpdatePortForward(number int, req AddPortForwardRequest) error {
	if !m.available() {
		return fmt.Errorf("netsh is not available")
	}
	if number < 1 {
	...
	if err := m.deletePortForwardRule(*target); err != nil {
		return err
	}
	if err := m.AddPortForward(req); err != nil {
		_ = m.addPortForwardWithoutValidation(portForwardRequestFromRule(*target))
		return err
	}
	return nil
}
...
func (m *netshManager) DeletePortForward(number int) error {
	...
	return m.deletePortForwardRule(*target)
}

func (m *netshManager) deletePortForwardRule(target PortForwardRule) error {
	proxyType := "v4tov4"
...
func (m *netshManager) addPortForwardWithoutValidation(req AddPortForwardRequest) error {
	listenAddr := strings.TrimSpace(req.ListenAddr)
	if listenAddr == "" {
		if strings.EqualFold(req.IPVersion, "ipv6") {
			listenAddr = "::"
		} else {
			listenAddr = "0.0.0.0"
		}
...
func (m *netshManager) KVMCompatibility() (KVMCompatStatus, error) {
	return KVMCompatStatus{}, nil
}

func (m *netshManager) EnsureKVMCompatibility() error {
	return nil
}
