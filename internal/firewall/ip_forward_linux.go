...
// enablePortForwardFamilyLocked persists the enabled state and turns on kernel forwarding
// for the given IP version. Caller must already hold ensureReady's lock (or call it before).
func (m *nftablesManager) enablePortForwardFamilyLocked(ipVersion string) error {
	persisted := loadPersistedPortForwardSettings()
	if ipVersion == "ipv6" {
		persisted.IPv6Enabled = true
	} else {
		persisted.IPv4Enabled = true
	}
	if err := savePersistedPortForwardSettings(persisted); err != nil {
		return err
	}
	return ensureKernelIPForward(ipVersion)
}
