...
func (m *nftablesManager) ensureLibvirtForwardChain() error {
	...
	return ensureLibvirtForwardChainRules(
		libvirtBridgeNames(),
		func(comment string) bool {
			return m.chainHasCommentInChain(nftChainForward, comment)
		},
		func(ifaceDirection, bridge, comment string) error {
			_, err := runCommandPrivileged("nft", "add", "rule", nftFamily, nftTable, nftChainForward,
				ifaceDirection, bridge, "accept", "comment", nftCommentValue(comment))
			if err != nil {
				return fmt.Errorf("add libvirt forward %s %s: %w", ifaceDirection, bridge, err)
			}
			return nil
		},
	)
...
// ensureLibvirtForwardChainRules ensures per-bridge forward accept rules for both
// directions. Extracted for regression tests around partial-rule self-healing.
func ensureLibvirtForwardChainRules(
	bridges []string,
	hasComment func(comment string) bool,
	addRule func(ifaceDirection, bridge, comment string) error,
) error {
	for _, bridge := range bridges {
		fwdComment := "termiscope-libvirt-fwd-" + bridge
		if !hasComment(fwdComment) {
			if err := addRule("iifname", bridge, fwdComment); err != nil {
				return err
			}
		}
		fwdOutComment := "termiscope-libvirt-fwd-out-" + bridge
		if !hasComment(fwdOutComment) {
			if err := addRule("oifname", bridge, fwdOutComment); err != nil {
				return err
			}
		}
	}
