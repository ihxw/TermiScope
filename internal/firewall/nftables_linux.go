...
func (m *nftablesManager) ensureReady() error {
	...
	if !commandExists("nft") {
		...
		if exported.Backend != "" {
			m.importLegacyRulesFrom(exported)
	...
	if !m.tableExists() {
		...
		if err := m.upgradeForwardChainPriority(); err != nil {
			return err
		}
		if err := m.upgradeInputChainPriority(); err != nil {
			return err
		}
		...
		if err := m.ensureSystemForwardDNATAcceptRule(); err != nil {
			return err
		}
...
// importLegacyRulesFrom imports rules from a previously exported firewall backend,
// sets the migration state, and writes the migration marker file.
func (m *nftablesManager) importLegacyRulesFrom(exported *exportedFirewall) {
...
func (m *nftablesManager) initTable(enabled bool) error {
	...
	chainSpec := fmt.Sprintf("{ type filter hook input priority filter - 1; policy %s; }", policy)
...
func (m *nftablesManager) ensurePortForwardDNATAcceptRule() error {
	if m.chainHasCommentInChain(nftChain, "termiscope-pf-dnat") {
		return nil
	}
...
// ensureSystemForwardDNATAcceptRule inserts ct status dnat accept into the
// system ip/ip6 filter FORWARD chains so DNAT-forwarded traffic survives
// third-party forward rules (libvirt, Docker) that run at priority 0 and may
// reject packets to virtual bridges.
func (m *nftablesManager) ensureSystemForwardDNATAcceptRule() error {
	comment := "termiscope-pf-dnat-fwd"
	for _, family := range []string{"ip", "ip6"} {
		chain := "FORWARD"
		if m.chainHasCommentInTableChain(family, "filter", chain, comment) {
			continue
		}
		_, err := runCommandPrivileged("nft", "insert", "rule", family, "filter", chain,
			"ct", "status", "dnat", "accept", "comment", nftCommentValue(comment))
		if err != nil {
			// Table or chain may not exist (no libvirt/Docker) — that's fine.
			if strings.Contains(err.Error(), "No such file") ||
				strings.Contains(err.Error(), "does not exist") ||
				strings.Contains(err.Error(), "not found") {
				continue
			}
			return fmt.Errorf("insert %s filter %s dnat accept rule: %w", family, chain, err)
		}
	}
	return nil
}
...
func (m *nftablesManager) ensureForwardChain() error {
	...
	// Add established/related accept so forwarded response traffic passes through
	// our chain before the system default forward chain (which may have DROP policy).
	if !m.chainHasCommentInChain(nftChainForward, "termiscope-fwd-established") {
		_, _ = runCommandPrivileged("nft", "add", "rule", nftFamily, nftTable, nftChainForward,
			"ct", "state", "{", "established,", "related", "}", "accept",
			"comment", nftCommentValue("termiscope-fwd-established"))
	}
	return nil
}

func (m *nftablesManager) upgradeForwardChainPriority() error {
	out, err := runCommandPrivileged("nft", "-a", "list", "chain", nftFamily, nftTable, nftChainForward)
	if err != nil {
		return nil // chain doesn't exist yet
	}
	if !strings.Contains(out, "hook forward") {
		return nil
	}
	// Already at the correct priority.
	if strings.Contains(out, "priority filter - 1") {
		return nil
	}
	// Save existing rule specs so we can re-add them after recreating the chain.
	var savedRules []string
	for _, line := range strings.Split(out, "\n") {
		line = strings.TrimSpace(line)
		if line == "" || strings.HasPrefix(line, "table ") || strings.HasPrefix(line, "chain ") ||
			strings.Contains(line, "type filter hook") || strings.Contains(line, "policy ") {
			continue
		}
		...
		if line != "" {
			savedRules = append(savedRules, line)
		}
	}
	_, _ = runCommandPrivileged("nft", "delete", "chain", nftFamily, nftTable, nftChainForward)
	if err := m.ensureForwardChain(); err != nil {
		return err
	}
	for _, rule := range savedRules {
		args := append([]string{"add", "rule", nftFamily, nftTable, nftChainForward}, strings.Fields(rule)...)
		_, _ = runCommandPrivileged("nft", args...)
	}
	return nil
}

func (m *nftablesManager) upgradeInputChainPriority() error {
	out, err := runCommandPrivileged("nft", "-a", "list", "chain", nftFamily, nftTable, nftChain)
	if err != nil {
		return nil // chain doesn't exist yet
	}
	...
	// Remember whether the chain was in drop mode so we can restore it.
	wasEnabled := strings.Contains(strings.ToLower(out), "policy drop")
	// Save existing rules.
	var savedRules []string
	for _, line := range strings.Split(out, "\n") {
		line = strings.TrimSpace(line)
	...
	_, _ = runCommandPrivileged("nft", "delete", "chain", nftFamily, nftTable, nftChain)
	// Recreate with new priority.
	policy := "accept"
	if wasEnabled {
		policy = "drop"
	}
	...
	for _, rule := range savedRules {
		args := append([]string{"add", "rule", nftFamily, nftTable, nftChain}, strings.Fields(rule)...)
		_, _ = runCommandPrivileged("nft", args...)
	}
	return nil
}
...
func (m *nftablesManager) buildPortForwardArgs(req AddPortForwardRequest) ([]string, error) {
	...
	// inet-family nftables requires explicit address family qualifier on dnat for IPv6 (and we
	// keep the IPv4 qualifier too for symmetry and clarity).
	if ipVersion == "ipv6" {
		args = append(args, "dnat", "ip6", "to", "["+targetIP+"]:"+targetPort)
	} else {
		args = append(args, "dnat", "ip", "to", targetIP+":"+targetPort)
	}
...
func (m *nftablesManager) ensurePortForwardSupportRule(rule PortForwardRule, chain, action string) error {
	...
	if line, ok := m.findChainRuleByComment(chain, comment); ok {
		if strings.Contains(line, "ct status dnat") {
			return nil
		}
		// Upgrade: replace pre-`ct status dnat` rule so we no longer touch traffic that wasn't
		// redirected through our DNAT rule.
...
func (m *nftablesManager) findChainRuleByComment(chain, comment string) (string, bool) {
...
func (m *nftablesManager) buildPortForwardSupportArgs(rule PortForwardRule, action string) ([]string, error) {
	...
	// `ct status dnat` ensures the forward/postrouting rule only matches connections that have
	// actually been redirected by our DNAT rule, instead of affecting any unrelated traffic that
	// happens to share the same target ip:port.
	args = append(args, "ct", "status", "dnat")
	if source != "" && source != "any" {
...
func (m *nftablesManager) addPortForwardInternal(req AddPortForwardRequest) error {
	...
	// rollbacks collects undo functions for side-effect rules that were newly added.
	// On any subsequent failure they are executed in reverse order to leave the
	// ruleset in a consistent state.
	var rollbacks []func()
	var retErr error
	defer func() {
		if retErr != nil {
	...
	if retErr = m.ensurePortForwardSupportRule(rule, nftChainForward, "accept"); retErr != nil {
		return retErr
	}
	if !forwardExisted {
		rollbacks = append(rollbacks, func() { m.deletePortForwardSupportRuleInChain(rule, nftChainForward) })
	}

	masqueradeExisted := m.chainHasCommentInChain(nftChainPostrouting, forwardComment)
	if retErr = m.ensurePortForwardSupportRule(rule, nftChainPostrouting, "masquerade"); retErr != nil {
		return retErr
	}
	if !masqueradeExisted {
		rollbacks = append(rollbacks, func() { m.deletePortForwardSupportRuleInChain(rule, nftChainPostrouting) })
	}

	outputExisted := m.chainHasCommentInChain(nftChainOutputNAT, portForwardOutputComment(rule))
	if retErr = m.ensurePortForwardOutputNATRule(req); retErr != nil {
		return retErr
	}
	if !outputExisted {
		rollbacks = append(rollbacks, func() { m.deletePortForwardOutputNATRule(rule) })
	}

	if _, retErr = runCommandPrivileged("nft", append([]string{"add", "rule", nftFamily, nftTable, nftChainPrerouting}, dnatArgs...)...); retErr != nil {
		return retErr
	}
	return m.persistRules()
}
...
func (m *nftablesManager) AddPortForward(req AddPortForwardRequest) error {
	...
	if !enabled {
		if err := m.enablePortForwardFamilyLocked(ipVersion); err != nil {
			return fmt.Errorf("auto-enable %s port forwarding: %w", ipVersion, err)
		}
	}
