...
Install the latest GitHub release directly with the published installer.

Linux:

```bash
curl -fsSL https://github.com/ihxw/TermiScope/releases/latest/download/install.sh | sudo bash
```
...
The default Linux/macOS command is interactive and prompts for install location, server port, JWT secret, and encryption key. For non-interactive installs with generated defaults:

```bash
curl -fsSL https://github.com/ihxw/TermiScope/releases/latest/download/install.sh | sudo bash -s -- -y
```

The macOS installer clears download quarantine metadata before registering the launchd service. Current builds are not Apple-notarized; if you manually download and open the binary from Finder, macOS may still show an Apple verification warning.

For an existing macOS install that already shows this warning:
...
```powershell
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; & ([scriptblock]::Create((New-Object Net.WebClient).DownloadString('https://github.com/ihxw/TermiScope/releases/latest/download/install.ps1')))
```

The default Windows command is interactive and prompts for install location, server port, JWT secret, and encryption key. Add `-Y` to the end of the PowerShell command for a non-interactive install with generated defaults.

The installer downloads the matching Linux, Windows, or macOS release package, installs TermiScope to `/opt/termiscope` on Linux/macOS or `C:\Program Files\TermiScope` on Windows, creates a startup service/task, and starts it. New installations use port `3000` by default; existing installations keep their current configuration.

For new installs, `configs/config.yaml` is generated with `allowed_origins` entries for `http://localhost:<port>`, `http://127.0.0.1:<port>`, and detected local IPv4 addresses using the selected port. Same-origin access through the built-in web server does not require CORS, but these defaults cover common hostname/IP switches during first setup.
