...
export function useSftpNameConflict() {
  ...
  const promptConflict = (name, isDir, forBatch = false) => {
	if (forBatch && batchPolicy === 'skip') return Promise.resolve('cancel')
	if (forBatch && batchPolicy === 'overwrite') return Promise.resolve('overwrite')
	if (forBatch && batchPolicy === 'keepBoth') return Promise.resolve('keepBoth')
  ...
  const resolveDestName = async (fileName, destFiles, { batch = false } = {}) => {
    if (!hasNameConflict(destFiles, fileName)) {
      return { destFileName: fileName, cancelled: false, overwrite: false }
    }
    ...
    if (action === 'cancel') {
      return { destFileName: fileName, cancelled: true, overwrite: false }
    }
    if (action === 'keepBoth') {
      return {
    ...
    return { destFileName: fileName, cancelled: false, overwrite: true }
  }
