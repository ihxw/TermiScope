<template>
  <div class="system-management">
            ...
            <a-spin :spinning="firewallLoading">
              <a-row :gutter="16" class="stats-row">
                ...
                <a-col :xs="24" :sm="8">
                  <a-statistic
                    ...
                    :value-style="firewallStatus.enabled ? { color: '#52c41a' } : { color: '#faad14' }"
                  />
                </a-col>
              </a-row>
              ...
              <a-tabs v-model:activeKey="firewallTab">
                ...
                <a-tab-pane key="forward" :tab="t('system.firewallTabForward')">
                  ...
                  <a-card size="small" :title="t('system.firewallPfSettingsTitle')" style="margin-bottom: 16px">
                    <a-row :gutter="[16, 12]">
                      <a-col :xs="24" :md="12">
                        <div class="pf-setting-row">
                          <span>{{ t('system.firewallPfEnableIPv4') }}</span>
                          <a-switch
                            ...
                            @change="savePortForwardSettings"
                          />
                        </div>
                        <div class="field-hint">
                          {{ t('system.firewallPfKernelState') }}:
                          {{ portForwardSettings.ipv4_ip_forward ? t('common.enabled') : t('common.disabled') }}
                        </div>
                      </a-col>
                      <a-col :xs="24" :md="12">
                        <div class="pf-setting-row">
                          <span>{{ t('system.firewallPfEnableIPv6') }}</span>
                          <a-switch
                        ...
                        <div class="field-hint">
                          {{ t('system.firewallPfKernelState') }}:
                          {{ portForwardSettings.ipv6_ip_forward ? t('common.enabled') : t('common.disabled') }}
                        </div>
                      </a-col>
                    </a-row>
                  </a-card>
                  <a-space wrap style="margin-bottom: 12px">
                    <a-button
                      type="dashed"
                      :disabled="!firewallStatus.available || !canAddPortForward"
                  ...
                  >
                    <template #bodyCell="{ column, record }">
                      <template v-if="column.key === 'forward'">
                        [{{ record.ip_version?.toUpperCase() }}]
                        {{ record.listen_address || (record.ip_version === 'ipv6' ? '::' : '0.0.0.0') }}:{{ record.listen_port }}
                        → {{ record.target_ip }}:{{ record.target_port }}
                      </template>
                      <template v-else-if="column.key === 'ops'">
                        <a-button
                          type="link"
    ...
    <a-modal
      ...
      :title="t('system.firewallAddRule')"
      :confirm-loading="firewallActionLoading"
      ...
      <a-form :model="firewallRuleForm" layout="vertical">
        <a-form-item :label="t('system.firewallRuleAction')" required>
          <a-select v-model:value="firewallRuleForm.action">
            ...
            <a-select-option value="reject">{{ t('system.firewallActionReject') }}</a-select-option>
          </a-select>
        </a-form-item>
    ...
    <a-modal
      ...
      :title="t('system.firewallAddPortForward')"
      :confirm-loading="firewallActionLoading"
      ...
      <a-form :model="portForwardForm" layout="vertical">
        <a-form-item :label="t('system.firewallPfIPVersion')" required>
          <a-radio-group v-model:value="portForwardForm.ip_version" @change="onPortForwardVersionChange">
            <a-radio-button value="ipv4" :disabled="!portForwardSettings.ipv4_enabled">IPv4</a-radio-button>
            <a-radio-button value="ipv6" :disabled="!portForwardSettings.ipv6_enabled">IPv6</a-radio-button>
          </a-radio-group>
        </a-form-item>
        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item :label="t('system.firewallPfListenAddr')" :extra="t('system.firewallPfListenAddrHelp')">
              <a-input
...
const VALID_SECTIONS = ['general', 'session', 'notification', 'data', 'maintenance', 'firewall']
...
watch(
  ...
  (section) => {
    if (section && VALID_SECTIONS.includes(section) && section !== activeSection.value) {
      ...
      if (section === 'firewall') fetchFirewall()
    }
  },
)
...
const orphanColumns = computed(() => [
  ...
  { title: t('system.orphanClientIPs'), key: 'client_ips' },
...
const canAddPortForward = computed(
  () => portForwardSettings.ipv4_enabled || portForwardSettings.ipv6_enabled,
)
...
const firewallColumns = computed(() => [
  ...
  { title: t('system.firewallColAction'), key: 'action', dataIndex: 'action', width: 90 },
  { title: t('system.firewallColPort'), dataIndex: 'port', key: 'port', width: 90 },
  { title: t('system.firewallColProtocol'), dataIndex: 'protocol', key: 'protocol', width: 90 },
  { title: t('system.firewallColSource'), dataIndex: 'source', key: 'source', ellipsis: true },
  { title: t('system.firewallColDirection'), dataIndex: 'direction', key: 'direction', width: 90 },
  { title: t('system.firewallColComment'), dataIndex: 'comment', key: 'comment', ellipsis: true },
  { title: t('common.actions'), key: 'ops', width: 90, fixed: 'right' },
])

const portForwardColumns = computed(() => [
  { title: '#', dataIndex: 'number', key: 'number', width: 60 },
  { title: t('system.firewallPfForward'), key: 'forward', ellipsis: true },
  { title: t('system.firewallColProtocol'), dataIndex: 'protocol', key: 'protocol', width: 80 },
  { title: t('system.firewallColSource'), dataIndex: 'source', key: 'source', ellipsis: true },
  { title: t('system.firewallColComment'), dataIndex: 'comment', key: 'comment', ellipsis: true },
  { title: t('common.actions'), key: 'ops', width: 90, fixed: 'right' },
])
...
const fetchFirewall = async () => {
  ...
  try {
    ...
    if (firewallStatus.available && !firewallStatus.enabled) {
      try {
        await api.post('/system/firewall/initialize')
      } catch {
        // non-fatal: status may still be readable
      }
    }
...
const openAddPortForward = () => {
  portForwardForm.ip_version = portForwardSettings.ipv4_enabled ? 'ipv4' : 'ipv6'
...
const savePortForwardSettings = async () => {
...
const submitPortForward = async () => {
  ...
  try {
    await api.post('/system/firewall/port-forwards', {
    ...
    message.error(err.response?.data?.error || t('system.firewallPortForwardAddFailed'))
  } finally {
...
const confirmDeletePortForward = (record) => {
  Modal.confirm({
    ...
    content: t('system.firewallPfDeleteConfirmContent', { number: record.number }),
...
const submitFirewallRule = async () => {
  firewallActionLoading.value = true
  try {
    await api.post('/system/firewall/rules', {
    ...
    message.error(err.response?.data?.error || t('system.firewallRuleAddFailed'))
  } finally {
    firewallActionLoading.value = false
  }
}
...
const confirmDeleteFirewallRule = (record) => {
  Modal.confirm({
    ...
    content: t('system.firewallDeleteConfirmContent', { number: record.number }),
    okText: t('common.confirm'),
    cancelText: t('common.cancel'),
    okType: 'danger',
    onOk: async () => {
...
const handleDisableFirewall = () => {
  Modal.confirm({
    ...
    content: t('system.firewallDisableConfirmContent'),
    okText: t('common.confirm'),
    cancelText: t('common.cancel'),
    okType: 'danger',
    onOk: async () => {
