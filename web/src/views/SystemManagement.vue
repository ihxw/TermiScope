...
const executeBackup = async () => {
  ...
  try {
    const request = { password: backupPassword.value }
    let response
    try {
      response = await api.post('/system/backup/create', request, { _silentError: true })
    } catch (err) {
      if (err.response?.status !== 404) throw err
      response = await api.post('/system/backup', request, { _silentError: true })
    }
    if (response?.filename && response?.ticket) {
      const fallbackPath = `/system/backup/download?file=${encodeURIComponent(response.filename)}&token=${encodeURIComponent(response.ticket)}`
      const downloadUrl = response.download_url
        ? new URL(response.download_url, window.location.origin).toString()
        : apiUrl(fallbackPath)
    ...
    message.error(t('system.backupFailed') + ': ' + errorMessage)
  } finally {
