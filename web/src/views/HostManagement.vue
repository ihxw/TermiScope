        ...
        >
          ...
          <template v-if="column.key === 'os'">
            <a-tooltip :title="getHostOsLabel(record)">
              <span class="host-os-icon" :class="`host-os-icon-${getHostOsType(record)}`">
                <WindowsOutlined v-if="getHostOsType(record) === 'windows'" />
                <AppleOutlined v-else-if="getHostOsType(record) === 'darwin'" />
                <CloudServerOutlined v-else-if="getHostOsType(record) === 'linux'" />
...
const getHostOsType = (host) => {
  const raw = String(host?.os_type || host?.os || '').toLowerCase()
