...
const handleEdit = async (host) => {
  ...
  try {
    ...
    const fullHost = await sshStore.fetchHost(host.id, { reveal: true, currentPassword })
    hostForm.value = {
      name: fullHost.name,
      host: fullHost.host,
      port: fullHost.port,
...
Invoke-WebRequest -Uri $url -Headers $headers -UseBasicParsing | Select-Object -ExpandProperty Content | Invoke-Expression`
  } else {
