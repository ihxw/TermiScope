<template>
  <div class="host-management-container">
    <a-alert
      v-if="authStore.isAdmin && orphanAgents.length && !orphanBannerDismissed"
      type="warning"
      show-icon
      closable
      class="orphan-banner"
      :message="t('monitor.orphanBannerTitle')"
      @close="orphanBannerDismissed = true"
    >
      <template #description>
        <span>{{ t('monitor.orphanBannerDesc', { count: orphanAgents.length }) }}</span>
        <a-button type="link" size="small" @click="goOrphanMaintenance">
          {{ t('monitor.orphanBannerAction') }}
        </a-button>
      </template>
    </a-alert>
      ...
      <div v-else class="table-wrapper">
        ...
        >
          ...
          <template v-if="column.key === 'drag'">
            <a-tooltip :title="isDragDisabled ? t('host.orderDragDisabled') : ''">
              <div :class="['drag-handle', { 'drag-handle-disabled': isDragDisabled }]">
                 <HolderOutlined />
              </div>
            </a-tooltip>
          </template>
          <template v-if="column.key === 'name'">
            <span class="host-name-cell">
          ...
          <template v-if="column.key === 'country'">
            <div v-if="getHostCountryCode(record)" class="host-country-cell">
              <CountryFlag :code="getHostCountryCode(record)" :size="14" />
              <span>{{ getHostCountryLabel(record) }}</span>
            </div>
            <span v-else class="empty-cell">-</span>
          </template>
          <template v-if="column.key === 'os'">
            <a-tooltip :title="getHostOsLabel(record)">
              <OsIcon :os="getHostOsValue(record)" :size="16" />
            </a-tooltip>
          </template>
          ...
          <template v-if="column.key === 'agent_transfer'">
            <div class="transfer-port-cell">
              <a-tooltip :title="getTransferPortStatusText(record)">
                <a-tag :color="getTransferPortColor(record)">
                  {{ record.agent_transfer_port || '-' }}
                </a-tag>
              </a-tooltip>
              <a-tooltip :title="record.agent_transfer_configurable ? t('host.setAgentTransferPort') : t('host.agentTransferUpdateRequired')">
                <a-button
                  type="text"
                  size="small"
                  class="transfer-port-settings"
                  :disabled="record.deleted_at || !record.agent_transfer_configurable"
                  @click="openTransferPortModal(record)"
                >
                  <SettingOutlined />
                </a-button>
              </a-tooltip>
          ...
          <template v-if="column.key === 'action'">
                ...
                <template #overlay>
                  <a-menu>
                    ...
                    <a-menu-item 
                      ...
                      @click="confirmPermanentDelete(record)"
                      style="color: #ff4d4f"
                    >
            ...
            <a-space v-else>
              ...
              <a-popconfirm
                v-if="!record.deleted_at"
                :title="t('host.deleteConfirm', { name: record.name })"
              ...
              <a-popconfirm
                v-else
                :title="t('host.permanentDeleteConfirm', { name: record.name })"
    ...
    <a-modal
      v-model:open="revealPasswordModalOpen"
      :title="t('host.revealPassword')"
      :confirmLoading="revealPasswordLoading"
      @ok="confirmRevealPassword"
      @cancel="cancelRevealPassword"
    >
      <a-form layout="vertical">
        <a-form-item :label="t('auth.password')" required>
          <a-input-password
            v-model:value="revealCurrentPassword"
            :placeholder="t('auth.passwordPlaceholder')"
            autocomplete="current-password"
            @pressEnter="confirmRevealPassword"
          />
        </a-form-item>
      </a-form>
    </a-modal>
    ...
    <a-modal
      v-model:open="transferPortModalOpen"
      :title="t('host.setAgentTransferPort')"
      :confirm-loading="transferPortSaving"
      @ok="confirmTransferPort"
    >
      <a-form layout="vertical">
        <a-form-item :label="t('host.agentTransferPort')" required>
          <a-input-number
            v-model:value="transferPortValue"
            :min="1"
            :max="65535"
            :precision="0"
            style="width: 100%"
            @pressEnter="confirmTransferPort"
          />
        </a-form-item>
        <a-alert
          v-if="transferPortHost && !transferPortHost.monitor_enabled"
          type="warning"
          show-icon
          :message="t('host.agentTransferRequiresAgent')"
        />
      </a-form>
    </a-modal>
    ...
    >
      <a-form :model="hostForm" layout="vertical">
        ...
        <template v-if="hostForm.host_type === 'control_monitor'">
          ...
          <a-form-item v-if="hostForm.auth_type === 'password'" :label="t('host.password')" :required="!editingHost">
            <a-space-compact style="width: 100%">
              <a-input-password
                ...
                :placeholder="editingHost ? t('host.placeholderKeepPassword') : t('host.placeholderPassword')"
                autocomplete="new-password"
              />
              <a-button
                v-if="editingHost"
                :loading="revealPasswordLoading"
                @click="handleRevealSavedPassword"
              >
                <EyeOutlined />
                {{ t('host.revealPassword') }}
              </a-button>
            </a-space-compact>
          </a-form-item>
          ...
          <a-form-item v-if="hostForm.auth_type === 'key'" :label="t('host.privateKey')" :required="!editingHost">
            <div style="display: flex; gap: 8px; margin-bottom: 8px;">
              ...
              >
                <a-button size="small">
                  <CloudUploadOutlined /> {{ t('host.uploadKey') || 'Upload Key File' }}
                </a-button>
              </a-upload>
            </div>
            <a-textarea
          ...
          <a-form-item :label="t('host.remoteShell')" :extra="t('host.remoteShellHelp')">
            <a-select v-model:value="hostForm.remote_shell">
              <a-select-option value="default">{{ t('host.remoteShellDefault') }}</a-select-option>
              <a-select-option value="powershell">{{ t('host.remoteShellPowershell') }}</a-select-option>
              <a-select-option value="pwsh">{{ t('host.remoteShellPwsh') }}</a-select-option>
              <a-select-option value="cmd">{{ t('host.remoteShellCmd') }}</a-select-option>
              <a-select-option value="bash">{{ t('host.remoteShellBash') }}</a-select-option>
            </a-select>
          </a-form-item>
        </template>

        <a-form-item :label="t('host.osType')" :extra="t('host.osTypeHelp')">
          <a-select v-model:value="hostForm.os_type">
            <a-select-option value="linux">{{ t('host.osTypeLinux') }}</a-select-option>
            <a-select-option value="darwin">{{ t('host.osTypeMacOS') }}</a-select-option>
            <a-select-option value="windows">{{ t('host.osTypeWindows') }}</a-select-option>
          </a-select>
        </a-form-item>
        ...
        <!-- Country Selection -->
        <a-form-item :label="t('host.country')">
          <a-select 
            v-model:value="hostForm.country_code" 
            show-search
            :placeholder="t('host.placeholderCountry')"
            :filter-option="filterCountryOption"
            allow-clear
          >
            <a-select-option value="">
              <span style="color: #999">{{ t('host.countryNone') }}</span>
            </a-select-option>
            <a-select-option v-for="c in countryOptions" :key="c.value" :value="c.value">
              <span :class="'fi fi-' + c.value" style="margin-right: 6px; width: 20px; height: 14px; display: inline-block; vertical-align: middle; line-height: 14px; border-radius: 1px"></span>
              <span style="vertical-align: middle">{{ getCountryLabel(c.value) }}</span>
            </a-select-option>
          </a-select>
        </a-form-item>
    ...
    >
      ...
      <div v-if="installCommandHost">
        ...
        <a-radio-group v-model:value="installPlatform" style="margin-bottom: 12px">
          ...
          <a-radio-button value="darwin">macOS</a-radio-button>
          <a-radio-button value="windows">Windows</a-radio-button>
        </a-radio-group>
        
        <p style="margin-bottom: 8px; color: #666; font-size: 13px">
          {{ installPlatform === 'windows' ? t('monitor.windowsInstallSupport') : installPlatform === 'darwin' ? t('monitor.macosInstallSupport') : t('monitor.installSupport') }}
        </p>
        <p v-if="installPlatform === 'windows'" class="install-command-hint">
          {{ t('monitor.windowsInstallCommandHint') }}
        </p>
...
import { getOrphanAgents } from '../api/monitor'
import { useI18n } from 'vue-i18n'
import { useMonitorStream } from '../composables/useMonitorStream'
import { deployMonitor, stopMonitor, batchDeployMonitor, batchStopMonitor, updateHostFingerprint, setAgentTransferPort } from '../api/ssh'
...
const delayedActionTimers = new Set()
const scheduleDelayedAction = (callback, delay) => {
	const timer = window.setTimeout(() => {
		delayedActionTimers.delete(timer)
		callback()
	}, delay)
	delayedActionTimers.add(timer)
}
...
const orphanBannerDismissed = ref(false)
...
const hostsLoadError = ref(false)
...
const handleKeyFileUpload = (file) => {
  ...
  reader.onload = (e) => {
    hostForm.value.private_key = e.target.result
    message.success(t('host.keyUploadSuccess') || 'Key loaded successfully')
  }
  reader.onerror = () => {
    message.error(t('host.keyReadFailed'))
  }
...
const revealPasswordModalOpen = ref(false)
const revealPasswordLoading = ref(false)
const revealCurrentPassword = ref('')
const revealPasswordResolver = ref(null)
const transferPortModalOpen = ref(false)
const transferPortSaving = ref(false)
const transferPortHost = ref(null)
const transferPortValue = ref(61725)
const transferPortRefreshTimers = new Set()

const deployVisible = ref(false)
const deployInsecure = ref(false)
const deployHost = ref(null)
const deploying = ref(false)
const deployAbortController = ref(null)
const deployRunId = ref(0)
...
const batchDeployAbortController = ref(null)
const batchDeployRunId = ref(0)
...
const filterCountryOption = (input, option) => {
  ...
  return getCountryLabel(value).toLowerCase().includes(query) || value.includes(query)
}
...
const getHostMonitorData = (host) => monitorStore.hostsById[Number(host?.id)]

const getHostCountryCode = (host) => (
  getHostMonitorData(host)?.country_code || host?.country_code || ''
)

const countryDisplayNames = computed(() => {
  try {
    return new Intl.DisplayNames([i18nLocale.value], { type: 'region' })
  } catch {
    return null
  }
})
...
const getCountryLabel = (code) => {
  ...
  const localizedName = countryDisplayNames.value?.of(normalizedCode)
  if (localizedName && localizedName !== normalizedCode) return localizedName

  return countryOptions.find(country => country.value === normalizedCode.toLowerCase())?.label || normalizedCode
}

const getHostCountryLabel = (host) => getCountryLabel(getHostCountryCode(host))

const getHostOsValue = (host) => (
  getHostMonitorData(host)?.os || host?.os || host?.os_type || ''
)

const getHostOsType = (host) => {
  const raw = String(getHostOsValue(host)).toLowerCase()
  if (raw.includes('windows') || raw === 'win') return 'windows'
  if (raw.includes('darwin') || raw.includes('mac')) return 'darwin'
  if (
    raw.includes('linux') ||
    raw.includes('openwrt') ||
    raw.includes('alpine') ||
    raw.includes('ubuntu') ||
    raw.includes('debian') ||
    raw.includes('centos') ||
...
const getHostOsLabel = (host) => {
  const type = getHostOsType(host)
  if (type === 'windows') return t('host.osTypeWindows')
  if (type === 'linux') return t('host.osTypeLinux')
  if (type === 'darwin') return t('host.osTypeMacOS')
  return t('host.osTypeUnspecified')
}

const getTransferPortColor = (host) => {
  if (host.agent_transfer_requested_port) return 'processing'
  if (host.agent_transfer_port_error) return 'error'
  return host.agent_transfer_available ? 'success' : 'default'
}

const getTransferPortStatusText = (host) => {
  if (host.agent_transfer_requested_port) {
    return t('host.agentTransferPortPending', { port: host.agent_transfer_requested_port })
  }
  if (host.agent_transfer_port_error) {
    return t('host.agentTransferPortFailed', { message: host.agent_transfer_port_error })
  }
  return host.agent_transfer_available
    ? t('host.agentTransferPortAvailable')
    : t('host.agentTransferPortUnavailable')
}

const openTransferPortModal = (host) => {
  transferPortHost.value = host
  transferPortValue.value = host.agent_transfer_requested_port || host.agent_transfer_port || 61725
  transferPortModalOpen.value = true
}

const scheduleTransferPortRefresh = () => {
  for (const delay of [2500, 12000]) {
    const timer = window.setTimeout(async () => {
      transferPortRefreshTimers.delete(timer)
      await loadHosts(true)
    }, delay)
    transferPortRefreshTimers.add(timer)
  }
}

const confirmTransferPort = async () => {
  const port = Number(transferPortValue.value)
  if (!Number.isInteger(port) || port < 1 || port > 65535 || !transferPortHost.value) {
    message.error(t('host.agentTransferPortInvalid'))
    return
  }
  transferPortSaving.value = true
  try {
    await setAgentTransferPort(transferPortHost.value.id, port)
    const record = sshStore.hosts.find(host => host.id === transferPortHost.value.id)
    if (record) {
      record.agent_transfer_requested_port = port
      record.agent_transfer_port_error = ''
    }
    transferPortModalOpen.value = false
    message.success(t('host.agentTransferPortQueued'))
    scheduleTransferPortRefresh()
  } catch (error) {
    message.error(error?.response?.data?.error || t('host.agentTransferPortSetFailed'))
  } finally {
    transferPortSaving.value = false
  }
}
...
const columns = computed(() => {
  const baseColumns = [
    ...
    { 
      ...
      sortOrder: sortedInfo.value.columnKey === 'id' ? sortedInfo.value.order : null
    },
    { title: t('host.os'), key: 'os', width: 52, align: 'center' },
    { 
      title: t('host.name'), 
      dataIndex: 'name', 
      key: 'name', 
    ...
    },
    {
      ...
      key: 'country',
      width: 120,
      sorter: (a, b) => getHostCountryLabel(a).localeCompare(getHostCountryLabel(b), i18nLocale.value),
      sortDirections: ['ascend', 'descend'],
      sortOrder: sortedInfo.value.columnKey === 'country' ? sortedInfo.value.order : null
    },
    { 
      title: t('host.host'), 
      dataIndex: 'host', 
    ...
    { title: t('host.agentTransferPort'), key: 'agent_transfer', width: 118, align: 'center' },
    { 
      title: t('host.type'), 
      key: 'type', 
      width: 100,
...
const cancelDeploy = () => {
    deployRunId.value += 1
    deployAbortController.value?.abort()
    deployAbortController.value = null
    if (deployHost.value) {
        monitorLoading.value[deployHost.value.id] = false
    }
    deploying.value = false
    deployVisible.value = false
    deployHost.value = null
    deployInsecure.value = false
}

const getFingerprintMismatch = (error) => {
  const data = error?.response?.data
  const fingerprint = data?.meta?.new_fingerprint
  return data?.code === 'fingerprint_mismatch' && typeof fingerprint === 'string' && fingerprint
    ? fingerprint
    : ''
}

const confirmFingerprintAndRetryDeploy = (host, fingerprint) => new Promise((resolve) => {
  Modal.confirm({
    title: t('monitor.fingerprintMismatchTitle'),
    content: t('monitor.fingerprintMismatchPrompt', { name: host.name, fingerprint }),
    okText: t('monitor.fingerprintAcceptAndDeploy'),
    cancelText: t('common.cancel'),
    onOk: async () => {
      try {
        await updateHostFingerprint(host.id, fingerprint)
        host.fingerprint = fingerprint
        message.success(t('monitor.fingerprintUpdated'))
        resolve(true)
      } catch (error) {
        message.error(t('monitor.fingerprintUpdateFailed') + ': ' + (error.response?.data?.error || error.message))
        throw error
      }
    },
...
const handleDeploy = async () => {
    ...
    const host = deployHost.value
    const runId = deployRunId.value + 1
    const controller = new AbortController()
    deployRunId.value = runId
    deployAbortController.value?.abort()
    deployAbortController.value = controller
    deploying.value = true
    monitorLoading.value[host.id] = true
    
    try {
        await deployMonitor(host.id, deployInsecure.value, { signal: controller.signal })
        if (deployRunId.value !== runId || controller.signal.aborted) return
        message.success(t('monitor.deploySuccess'))
        host.monitor_enabled = true
        deployVisible.value = false
    } catch (error) {
        if (isCancellationError(error) || deployRunId.value !== runId) return
        const fingerprint = getFingerprintMismatch(error)
        if (fingerprint) {
            const accepted = await confirmFingerprintAndRetryDeploy(host, fingerprint)
            if (!accepted || deployRunId.value !== runId || controller.signal.aborted) return
            ...
            try {
                ...
                if (isCancellationError(retryError) || deployRunId.value !== runId) return
                message.error(t('monitor.deployFailed') + ': ' + (retryError.response?.data?.error || retryError.message))
            }
            return
        }
        message.error(t('monitor.deployFailed') + ': ' + (error.response?.data?.error || error.message))
    } finally {
        if (deployRunId.value === runId) {
            deploying.value = false
...
const isDragDisabled = computed(() => {
  const isSearchActive = !!searchText.value
  const isDeletedViewActive = showDeleted.value
  const isQuickFilterActive = quickFilter.value !== 'all'
  const isColumnSortActive = !!(sortedInfo.value && sortedInfo.value.columnKey && sortedInfo.value.order)
  const isColumnFilterActive = !!(filteredInfo.value && Object.values(filteredInfo.value).some(val => val && val.length > 0))
  
  return isSearchActive || isDeletedViewActive || isQuickFilterActive || isColumnSortActive || isColumnFilterActive
})

const initSortable = () => {
  const tableWithBody = document.querySelector('.ant-table-tbody')
  if (!tableWithBody) return

  if (tableWithBody._sortable) {
    tableWithBody._sortable.destroy()
    tableWithBody._sortable = null
  }
  ...
  tableWithBody._sortable = Sortable.create(tableWithBody, {
    ...
    onEnd: async (evt) => {
      ...
      try {
          ...
          if (userId && localStorage.getItem('termScope_host_order_pending_' + userId) === 'true') {
              message.warning(t('host.orderStoredLocally'))
          } else {
...
watch([isDragDisabled, () => sshStore.hosts], () => {
  nextTick(() => {
    initSortable()
  })
}, { deep: true, immediate: true })

const loadOrphanAgents = async () => {
...
const goOrphanMaintenance = () => {
  router.push({ name: 'SystemManagement', query: { section: 'maintenance' } })
}
...
const checkAllStatuses = async () => {
  ...
  const checks = activeHosts.map(async (host) => {
    ...
    try {
      ...
      hostStatuses.value[host.id] = { status: 'offline', error: t('host.statusCheckFailed') }
    }
  })
...
const loadHosts = async (force = false) => {
  loading.value = true
  hostsLoadError.value = false
  try {
    const filters = {}
    ...
    await sshStore.fetchHosts(filters, { force })
    checkAllStatuses()
  } catch (error) {
    hostsLoadError.value = true
  } finally {
    loading.value = false
  }
}
...
const requestRevealPassword = () => {
  revealCurrentPassword.value = ''
  revealPasswordModalOpen.value = true
  return new Promise((resolve) => {
    revealPasswordResolver.value = resolve
  })
}

const confirmRevealPassword = () => {
  if (!revealCurrentPassword.value) {
    message.error(t('auth.passwordRequired'))
    return
  }

  revealPasswordModalOpen.value = false
  const resolve = revealPasswordResolver.value
  revealPasswordResolver.value = null
  resolve?.(revealCurrentPassword.value)
  revealCurrentPassword.value = ''
}

const cancelRevealPassword = () => {
  revealPasswordModalOpen.value = false
  const resolve = revealPasswordResolver.value
  revealPasswordResolver.value = null
  resolve?.('')
...
const handleEdit = async (host) => {
  ...
  try {
    ...
    hostForm.value = {
      ...
      remote_shell: fullHost.remote_shell || 'default',
      os_type: fullHost.os_type || 'linux',
      ...
      flag: fullHost.flag || '',
      country_code: fullHost.country_code || ''
    }
    showModal.value = true
  } catch (error) {
...
const handleRevealSavedPassword = async () => {
  ...
  const currentPassword = await requestRevealPassword()
  if (!currentPassword) return

  revealPasswordLoading.value = true
  try {
    const fullHost = await sshStore.fetchHost(editingHost.value.id, { reveal: true, currentPassword })
    if (!fullHost.password) {
      message.info(t('host.noSavedPassword'))
      return
    }
    hostForm.value.password = fullHost.password
    message.success(t('host.passwordRevealed'))
  } catch (error) {
    message.error(t('host.revealPasswordFailed'))
  } finally {
    revealPasswordLoading.value = false
  }
}
...
const confirmDelete = (host) => {
  Modal.confirm({
    title: t('host.deleteConfirm', { name: host.name }),
    okText: t('common.confirm'),
    cancelText: t('common.cancel'),
    okType: 'danger',
...
const confirmPermanentDelete = (host) => {
  Modal.confirm({
    title: t('host.permanentDeleteConfirm', { name: host.name }),
    okText: t('common.confirm'),
    cancelText: t('common.cancel'),
    okType: 'danger',
    onOk: () => handlePermanentDelete(host.id)
  })
}
...
const cancelBatchDeploy = () => {
  batchDeployRunId.value += 1
  batchDeployAbortController.value?.abort()
  batchDeployAbortController.value = null
  batchDeploying.value = false
  batchDeployVisible.value = false
  deployStatus.value = {}
  batchDeployInsecure.value = false
}

const handleBatchDeploy = async () => {
  const runId = batchDeployRunId.value + 1
  const controller = new AbortController()
  batchDeployRunId.value = runId
  batchDeployAbortController.value?.abort()
  batchDeployAbortController.value = controller
  ...
  if (validHostIds.length === 0) {
    ...
    if (batchDeployRunId.value === runId) {
      batchDeploying.value = false
      batchDeployAbortController.value = null
    }
    return
  ...
  try {
    await batchDeployMonitor(validHostIds, batchDeployInsecure.value, (result) => {
        if (batchDeployRunId.value !== runId || controller.signal.aborted) return
        // Real-time update
    ...
    if (failCount === 0) {
	  ...
	  scheduleDelayedAction(() => {
        if (batchDeployRunId.value !== runId) return
        batchDeployVisible.value = false
    ...
    if (isCancellationError(error) || batchDeployRunId.value !== runId) return
...
const openInstallCommandModal = async (host) => {
  const hostOsType = getHostOsType(host)
  installPlatform.value = hostOsType === 'windows' ? 'windows' : hostOsType === 'darwin' ? 'darwin' : 'linux'
  installCommandVisible.value = true
  try {
    installCommandHost.value = await sshStore.fetchHost(host.id)
  } catch {
...
const getInstallCommand = () => {
  ...
  if (installPlatform.value === 'windows') {
    // Windows PowerShell command (secret in header - not logged in URL)
    return `$headers = @{ Authorization = "Bearer ${secret}" }
$url = "${serverUrl}/api/monitor/install?host_id=${hostId}&os=windows"
...
try {
    ...
    // Linux/macOS shell command (secret in header — not logged in URL)
    return `curl -fsSL -H "Authorization: Bearer ${secret}" "${serverUrl}/api/monitor/install?host_id=${hostId}" | bash`
  }
}
...
const openUninstallCommandModal = async (host) => {
...
const getUninstallCommand = () => {
  ...
  const secret = installCommandHost.value.monitor_secret || ''
	...
	return `curl -fsSL -H "Authorization: Bearer ${secret}" "${serverUrl}/api/monitor/uninstall?host_id=${hostId}" | bash`
}
...
onUnmounted(() => {
  ...
  cancelDeploy()
  cancelBatchDeploy()
  for (const timer of transferPortRefreshTimers) window.clearTimeout(timer)
  transferPortRefreshTimers.clear()
	for (const timer of delayedActionTimers) window.clearTimeout(timer)
...
.host-country-cell > span:last-child {
  overflow: hidden;
...
.transfer-port-cell :deep(.ant-tag) {
  min-width: 54px;
  margin: 0;
  text-align: center;
}

.transfer-port-settings {
  width: 24px;
  height: 24px;
  padding: 0;
}
