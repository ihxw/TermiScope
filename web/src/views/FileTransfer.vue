<template>
  <div class="file-transfer-page">
    ...
    <TransferQueue ref="transferQueueRef" :dock-bottom="72" @busy-change="crossTransferBusy = $event" @cancel="cancelTransfer" />
...
const runTransfer = async ({
  ...
  try {
    await transferFile(
      ...
      'copy',
      { destFileName, overwrite, signal: controller.signal },
    )
...
const handleTransfer = async (side, data) => {
  ...
  const { destFileName, cancelled, overwrite } = await resolveDestName(data.name, getDestFiles(destBrowser))
  if (cancelled) {
    return
  }
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      ...
      for (const key of selectedKeys) {
        ...
        const { destFileName, cancelled, overwrite } = await resolveDestName(
          fileName,
          destFilesSnapshot(),
          { batch: true }
        )
