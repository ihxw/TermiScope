import { defineStore } from 'pinia'

const AGENT_UPDATE_PROGRESS_EVENTS = new Set(['command', 'update_downloading', 'update_installing'])
const AGENT_UPDATE_TERMINAL_EVENTS = new Set(['update_failed', 'update_idle', 'update_complete'])

function emptyMetrics(hostId, sh) {
    return {
        host_id: hostId,
        hostname: '',
        os: sh?.os_type || '',
        os_type: sh?.os_type || '',
        uptime: 0,
        cpu: 0,
        cpu_count: 0,
        cpu_model: '',
        cpu_mhz: 0,
        mem_used: 0,
        mem_total: 0,
        disk_used: 0,
        disk_total: 0,
        net_rx: 0,
        net_tx: 0,
        net_monthly_rx: 0,
        net_monthly_tx: 0,
        net_traffic_limit: sh?.net_traffic_limit ?? 0,
        net_traffic_used_adjustment: sh?.net_traffic_used_adjustment ?? 0,
        net_traffic_counter_mode: sh?.net_traffic_counter_mode || 'total',
        net_reset_day: sh?.net_reset_day ?? 1,
        net_last_reset_date: sh?.net_last_reset_date || '',
        notify_offline_enabled: sh?.notify_offline_enabled,
        notify_traffic_enabled: sh?.notify_traffic_enabled,
        notify_offline_threshold: sh?.notify_offline_threshold,
        notify_traffic_threshold: sh?.notify_traffic_threshold,
        notify_channels: sh?.notify_channels,
        expiration_date: sh?.expiration_date,
        billing_period: sh?.billing_period,
        billing_amount: sh?.billing_amount,
        currency: sh?.currency,
        flag: sh?.flag,
        country_code: sh?.country_code || '',
        agent_version: sh?.agent_version || '',
        _offline: true,
        _clientLastUpdated: 0,
    }
}

function mergeSSHConfig(existing, sh) {
    return {
        ...existing,
        os: existing.os || sh.os_type || '',
        os_type: sh.os_type || existing.os_type || '',
        notify_offline_enabled: sh.notify_offline_enabled,
        notify_traffic_enabled: sh.notify_traffic_enabled,
        notify_offline_threshold: sh.notify_offline_threshold,
        notify_traffic_threshold: sh.notify_traffic_threshold,
        notify_channels: sh.notify_channels,
        expiration_date: sh.expiration_date,
        billing_period: sh.billing_period,
        billing_amount: sh.billing_amount,
        currency: sh.currency,
        flag: sh.flag,
        country_code: sh.country_code ?? existing.country_code ?? '',
        net_reset_day: sh.net_reset_day,
        net_last_reset_date: sh.net_last_reset_date,
        net_traffic_limit: sh.net_traffic_limit,
        net_traffic_used_adjustment: sh.net_traffic_used_adjustment,
        net_traffic_counter_mode: sh.net_traffic_counter_mode,
        agent_version: sh.agent_version || existing.agent_version || '',
    }
}

/** Live monitor metrics keyed by host_id (WebSocket + SSH config). */
export const useMonitorStore = defineStore('monitor', {
    state: () => ({
        hostsById: {},
        streamPaused: false,
        connected: false,
        serverAgentVersion: null,
    }),

    getters: {
        hostsList: (state) => Object.values(state.hostsById),
    },

    actions: {
        setStreamPaused(paused) {
            this.streamPaused = paused
        },

        setConnected(v) {
            this.connected = v
        },

        setServerAgentVersion(version) {
            this.serverAgentVersion = version
        },

        syncConfigFromSSH(sshHosts) {
            if (!Array.isArray(sshHosts)) return
            const enabledIds = new Set()

            for (const sh of sshHosts) {
                if (!sh.monitor_enabled) continue
                const id = Number(sh.id)
                enabledIds.add(id)
                const existing = this.hostsById[id]
                if (existing) {
                    this.hostsById[id] = mergeSSHConfig(existing, sh)
                } else {
                    this.hostsById[id] = emptyMetrics(id, sh)
                }
            }

            for (const key of Object.keys(this.hostsById)) {
                const id = Number(key)
                if (!enabledIds.has(id)) {
                    delete this.hostsById[id]
                }
            }
        },

        applyUpdates(updates) {
            if (!updates) return
            const list = Array.isArray(updates) ? updates : [updates]
            const now = Date.now()
            const clean = (v) => (v || '').toString().replace(/^v/, '').trim()
            const serverV = clean(this.serverAgentVersion)

            for (const rawUpdate of list) {
                if (!rawUpdate || rawUpdate.host_id === undefined || rawUpdate.host_id === null) continue
                const id = Number(rawUpdate.host_id)
                const update = {
                    ...rawUpdate,
                    host_id: id,
                    _offline: false,
                    _clientLastUpdated: now,
                }
                const prev = this.hostsById[id] || { host_id: id }
                const merged = { ...prev, ...update }

                // 旧版 Agent 或地理位置检测失败时会上报空值，不能覆盖主机配置。
                if (!update.country_code) {
                    merged.country_code = prev.country_code || ''
                }

                // 静态系统信息尚未上报时保留主机配置类型，避免卡片图标变为空白。
                if (!update.os) {
                    merged.os = prev.os || prev.os_type || ''
                }

                if (
                    merged.agent_update_status &&
                    serverV &&
                    clean(merged.agent_version) === serverV
                ) {
                    merged.agent_update_status = null
                }

                this.hostsById[id] = merged
            }
        },

        handleStreamMessage(msg) {
            if (!msg?.type) return
            if (msg.type === 'init' || msg.type === 'update') {
                this.applyUpdates(msg.data)
            } else if (msg.type === 'offline') {
                this.markOffline(msg.data)
            } else if (msg.type === 'remove') {
                this.removeHost(msg.data)
            } else if (msg.type === 'agent_event') {
                this.handleAgentEvent(msg.data)
            }
        },

        markOffline(hostId) {
            const id = Number(hostId)
            const existing = this.hostsById[id]
            if (!existing) return

            this.hostsById[id] = {
                ...existing,
                status: 'offline',
                net_rx_rate: 0,
                net_tx_rate: 0,
                interfaces: Array.isArray(existing.interfaces)
                    ? existing.interfaces.map((iface) => ({
                        ...iface,
                        rx_rate: 0,
                        tx_rate: 0,
                    }))
                    : existing.interfaces,
                _offline: true,
                _clientLastUpdated: 0,
            }
        },

        removeHost(hostId) {
            delete this.hostsById[Number(hostId)]
        },

        handleAgentEvent(event) {
            const id = Number(event?.host_id)
            if (!this.hostsById[id]) return

            if (!AGENT_UPDATE_PROGRESS_EVENTS.has(event.event) && !AGENT_UPDATE_TERMINAL_EVENTS.has(event.event)) return

            this.hostsById[id] = {
                ...this.hostsById[id],
                agent_update_status: AGENT_UPDATE_PROGRESS_EVENTS.has(event.event) ? event.message : null,
            }
        },

        patchHost(hostId, patch) {
            const id = Number(hostId)
            this.hostsById[id] = {
                ...(this.hostsById[id] || { host_id: id }),
                ...patch,
                host_id: id,
            }
        },

        clear() {
            this.hostsById = {}
			this.streamPaused = false
			this.connected = false
			this.serverAgentVersion = null
        },
    },
})
