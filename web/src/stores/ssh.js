    ...
    actions: {
        async fetchHosts(filters = {}, { force = false } = {}) {
            ...
            if (
                !force &&
                this.hosts.length > 0 &&
				this.hostsFiltersKey === filtersKey &&
                Date.now() - this.hostsFetchedAt < this.hostsFetchTTL
            ) {
                return this.hosts
            }
            if (this.hostsFetchPromise && this.hostsFiltersKey === filtersKey) {
                return this.hostsFetchPromise
            }

            this.loading = true
			this.hostsFiltersKey = filtersKey
			const generation = this.hostsGeneration
			const requestedUserID = useAuthStore().user?.id
            this.hostsFetchPromise = (async () => {
                try {
					...
					if (generation !== this.hostsGeneration || requestedUserID !== useAuthStore().user?.id) {
						return []
					}
                    ...
                    this.loading = false
					if (generation === this.hostsGeneration) this.hostsFetchPromise = null
                }
            })()
            return this.hostsFetchPromise
        },
		...
		clearUserState() {
			this.hostsGeneration += 1
			this.hosts = []
			this.hostsFetchedAt = 0
			this.hostsFiltersKey = ''
