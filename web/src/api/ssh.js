...
export const getHost = async (id, { reveal = false, currentPassword = '' } = {}) => {
    const params = new URLSearchParams()
    if (reveal) params.set('reveal', 'true')
    if (currentPassword) params.set('current_password', currentPassword)
    const query = params.toString()
    return await api.get(`/ssh-hosts/${id}${query ? '?' + query : ''}`)
}
...
export const deployMonitor = async (id, insecure = false, options = {}) => {
    return await api.post(`/ssh-hosts/${id}/monitor/deploy`, { insecure }, options)
}
...
export const setAgentTransferPort = async (id, port) => {
    return await api.put(`/ssh-hosts/${id}/monitor/transfer-port`, { port })
}
...
export const getHostTrafficHistory = async (hostId, range = '24h') => {
    return await api.get(`/ssh-hosts/${hostId}/monitor/traffic-history`, { params: { range } })
}
...
export const batchDeployMonitor = async (hostIds, insecure = false, onResult, options = {}) => {
    const response = await fetchWithAuth('/api/ssh-hosts/monitor/batch-deploy', {
        method: 'POST',
        headers: {
    ...
    await consumeNDJSON(response.body, {
        signal: options.signal,
        onEvent: onResult,
        onInvalidLine: (line, error) => console.error('JSON parse error:', error, line),
    })
}

export const batchStopMonitor = async (hostIds, onResult) => {
    const response = await fetchWithAuth('/api/ssh-hosts/monitor/batch-stop', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
