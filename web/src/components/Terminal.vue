...
const handleGlobalCommandShortcut = (event) => {
  ...
  if (!commandsDropdownOpen.value) return
  if (!/^[1-9]$/.test(event.key)) return
  const index = Number(event.key) - 1
  if (index >= commandTemplates.value.length) return
  event.preventDefault()
  event.stopPropagation()
...
const handleTerminalKeyEvent = (event) => {
  ...
  if (event.type === 'keydown' && event.altKey && event.key.toLowerCase() === 'k') {
    event.preventDefault()
    showCommandsDropdown()
    return false
  }
...
const trackUserExitInput = (data) => {
  ...
  for (const char of data) {
    ...
    if (char === '\r' || char === '\n') {
      const command = pendingInputLine.trim()
    ...
    if (char === '\u007f' || char === '\b') {
      pendingInputLine = pendingInputLine.slice(0, -1)
      continue
    }
    if (char >= ' ' && char !== '\x7f') {
      pendingInputLine += char
    }
  }
}
...
const connectWebSocket = async () => {
  ...
  try {
    ...
    socket.onerror = (error) => {
      ...
      if (manualDisconnected.value || sessionEnded.value) {
    ...
    socket.onclose = (event) => {
      ...
      const endedQuietly = manualDisconnected.value || sessionEnded.value || event.code === 1000 || event.code === 1005
      if (detail && !endedQuietly) {
        console.warn('[Terminal] WebSocket closed:', event.code, event.reason || detail)
        message.error(detail)
        terminal.value?.writeln(`\r\n\x1b[31m${detail}\x1b[0m\r\n`)
      } else if (terminal.value && !manualDisconnected.value) {
        terminal.value.writeln(`\r\n\x1b[33m${t('terminal.disconnected')}\x1b[0m\r\n`)
      }
      if (ws.value === socket) {
        ws.value = null
      }
      // UX: Auto-reconnect on abnormal closure
      if (!manualDisconnected.value && !sessionEnded.value && event.code !== 1000 && event.code !== 1005) {
        message.info(t('terminal.reconnecting') || 'Auto-reconnecting...')
        setTimeout(() => {
          if (!manualDisconnected.value && !sessionEnded.value && connectionStatus.value === 'Disconnected') {
            reconnect()
          }
        }, 3000)
      }
...
const handleVisibilityChange = () => {
  ...
  nextTick(() => {
    handleResize()
    if (!manualDisconnected.value && !sessionEnded.value && props.active && connectionStatus.value === 'Disconnected') {
      reconnect()
    }
  })
}
