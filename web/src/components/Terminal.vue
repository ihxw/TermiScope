        ...
        <a-dropdown :disabled="connectionStatus !== 'Connected'" placement="topRight" @openChange="onCommandsDropdownOpen">
          ...
          <template #overlay>
            <a-menu @click="handleQuickCommand">
              ...
              <a-menu-item v-for="cmd in commandTemplates" :key="String(cmd.id)">
...
const handleQuickCommand = ({ key }) => {
  ...
  if (command && ws.value && ws.value.readyState === WebSocket.OPEN) {
    const data = command.command + (command.auto_enter ? '\n' : '')
...
const connectWebSocket = async () => {
  ...
  try {
    ...
    socket.onclose = (event) => {
      ...
      if (!manualDisconnected.value && event.code !== 1000 && event.code !== 1005) {
        message.info(t('terminal.reconnecting') || 'Auto-reconnecting...')
        setTimeout(() => {
          if (!manualDisconnected.value && connectionStatus.value === 'Disconnected') {
            reconnect()
          }
        }, 3000)
      }
...
const handleVisibilityChange = () => {
  ...
  nextTick(() => {
    handleResize()
    if (!manualDisconnected.value && props.active && connectionStatus.value === 'Disconnected') {
      reconnect()
    }
  })
}
