<template>
  <div class="terminal-wrapper" :style="{ 
    ...
    <a-modal
      :open="terminalLinkDialog.open"
      :title="t('terminal.linkDialogTitle')"
      :mask-closable="false"
      @cancel="closeTerminalLinkDialog"
    >
      <p class="terminal-link-prompt">{{ t('terminal.linkDialogPrompt') }}</p>
      <div class="terminal-link-url" :class="{ 'terminal-link-url-dark': themeStore.isDark }">
        {{ terminalLinkDialog.uri }}
      </div>
      <template #footer>
        <a-button @click="closeTerminalLinkDialog">{{ t('common.cancel') }}</a-button>
        <a-button :loading="terminalLinkDialog.copying" @click="copyTerminalLink">
          {{ t('terminal.copyLink') }}
        </a-button>
        <a-button type="primary" @click="openTerminalLink">
          {{ t('terminal.openLink') }}
        </a-button>
      </template>
    </a-modal>
...
const terminalLinkDialog = reactive({
  open: false,
  uri: '',
  copying: false,
})
...
const toggleSelectionMode = () => {
  ...
  if (isSelectionMode.value) {
    message.success(t('terminal.selectionModeEnabled'))
  } else {
    message.info(t('terminal.selectionModeDisabled'))
  }
}
...
let suppressPointerModeInputUntil = 0
...
const focusTerminal = () => {
  nextTick(() => terminal.value?.focus())
}

const handleTerminalLinkClick = (event, uri) => {
  event?.preventDefault()
  // xterm releases its drag-selection state from a document-level mouseup handler.
  // Let this mouseup keep bubbling and remove the temporary selection from the click.
  terminal.value?.clearSelection()
  terminalLinkDialog.uri = uri
  terminalLinkDialog.open = true
}

const closeTerminalLinkDialog = () => {
  terminalLinkDialog.open = false
  terminalLinkDialog.uri = ''
  terminalLinkDialog.copying = false
  focusTerminal()
}

const openTerminalLink = () => {
  const uri = terminalLinkDialog.uri
  if (!uri) return
	let parsed
	try {
		parsed = new URL(uri)
	} catch {
		message.error(t('terminal.invalidLink'))
		return
	}
  ...
  closeTerminalLinkDialog()
}

const copyTerminalLink = async () => {
  const uri = terminalLinkDialog.uri
  if (!uri || terminalLinkDialog.copying) return
  terminalLinkDialog.copying = true
  try {
    await navigator.clipboard.writeText(uri)
    message.success(t('terminal.linkCopied'))
    closeTerminalLinkDialog()
  } catch (err) {
    terminalLinkDialog.copying = false
...
const initTerminal = () => {
  ...
  terminal.value = new Terminal({
    ...
    linkHandler: {
      activate: handleTerminalLinkClick,
    },
  })
  ...
  terminal.value.onData((data) => {
    if (Date.now() <= suppressPointerModeInputUntil && isTerminalPointerModeReport(data)) {
      return
    }
    trackCdCommand(data)
...
const connectWebSocket = async () => {
	if (wsConnectInFlight || componentDisposed) return
  ...
  try {
    ...
    socket.onerror = (error) => {
      ...
      // An error after onopen is normally followed by onclose. Let onclose
      // classify it as a recoverable transport failure instead of turning it
      // into a permanent terminal error.
...
const resetTerminalLocalInputModes = async () => {
  if (!terminal.value) return
  suppressPointerModeInputUntil = Date.now() + STALE_POINTER_MODE_INPUT_SUPPRESS_MS
  await new Promise((resolve) => {
    ...
    try {
      terminal.value.write(TERMINAL_LOCAL_INPUT_MODE_RESET_SEQUENCE, finish)
    } catch (error) {
      finish()
    }
  })
...
const reconnect = async () => {
  ...
  await resetTerminalLocalInputModes()
  handleResize()
...
.terminal-link-url {
  ...
  font-family: 'TermiScope Mono', monospace;
  font-size: 12px;
  line-height: 1.6;
...
/* FitAddon controls the pixel grid; only constrain and clip its paint area here. */
:deep(.xterm) {
  max-width: 100%;
  max-height: 100%;
  overflow: hidden;
...
:deep(.xterm-viewport) {
  ...
  /* xterm 按始终存在的纵向滚动条计算列数，避免长输出出现后遮住最右侧字符。 */
