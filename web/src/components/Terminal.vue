...
} from '../utils/terminalReconnectPolicy'
...
const connectionErrorSeen = ref(false)
...
const connectWebSocket = async () => {
  ...
  connectionErrorSeen.value = false
  ...
  try {
    ...
    socket.onmessage = (event) => {
      ...
      try {
        ...
        if (msg && typeof msg === 'object' && msg.type) {
          if (msg.type === 'error') {
            connectionErrorSeen.value = true
    ...
    socket.onclose = (event) => {
      ...
      const shouldReconnect = shouldAutoReconnectTerminalClose(event, {
        manualDisconnected: manualDisconnected.value,
        sessionEnded: sessionEnded.value,
        connectionErrorSeen: connectionErrorSeen.value,
      })
      ...
      } else if (connectionErrorSeen.value && terminalErrorWritten.value) {
        connectionStatus.value = 'Error'
      ...
      } else if (connectionErrorSeen.value) {
        connectionStatus.value = 'Error'
