<template>
  <a-modal
    ...
    :mask-closable="!renameLoading"
    :cancel-button-props="{ disabled: renameLoading }"
    @update:open="$emit('update:renameVisible', $event)"
    @after-open-change="handleRenameOpenChange"
    @ok="$emit('rename-ok')"
  >
    <a-form-item :validate-status="renameError ? 'error' : ''" :help="renameError" style="margin-bottom: 0">
      <a-input
        ref="renameInputRef"
        :value="renameName"
        :disabled="renameLoading"
        :placeholder="t('sftp.newName')"
        @update:value="$emit('update:renameName', $event)"
      />
    </a-form-item>
  </a-modal>
  ...
  <a-modal
    ...
    @after-open-change="handleCreateOpenChange"
    @ok="$emit('create-ok')"
  >
    <a-form-item :validate-status="createError ? 'error' : ''" :help="createError" style="margin-bottom: 0">
      <a-input
        ref="createInputRef"
        :value="createName"
  ...
  >
    <div v-if="previewLoading" style="text-align: center; padding: 60px 20px">
      <a-spin :tip="t('sftp.mediaLoading')" />
    </div>
    <div
      v-else-if="previewSrc"
      style="display: flex; justify-content: center; align-items: center; background: #000; min-height: 320px; max-height: 75vh; border-radius: 6px; overflow: hidden; padding: 8px;"
    >
      <video
...
const handleRenameOpenChange = (open) => {
  if (open) {
    nextTick(() => {
