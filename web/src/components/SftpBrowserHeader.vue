    ...
    <div class="toolbar-row toolbar-main">
      ...
      <template v-if="!pathInputVisible">
        <div class="breadcrumb-container" @click="$emit('show-path-input')">
          ...
          <div ref="pathBreadcrumbViewportRef" class="path-breadcrumb-viewport">
...
const pathBreadcrumbViewportRef = ref(null)
let pathBreadcrumbResizeObserver = null

const scrollPathToCurrentFolder = () => {
  nextTick(() => {
    const viewport = pathBreadcrumbViewportRef.value
    if (viewport) viewport.scrollLeft = viewport.scrollWidth
  })
}

watch(() => props.pathParts.join('/'), scrollPathToCurrentFolder, { immediate: true })
watch(pathBreadcrumbViewportRef, (viewport, previousViewport) => {
  if (!pathBreadcrumbResizeObserver) return
  if (previousViewport) pathBreadcrumbResizeObserver.unobserve(previousViewport)
  if (viewport) pathBreadcrumbResizeObserver.observe(viewport)
  scrollPathToCurrentFolder()
})

onMounted(() => {
  if (typeof ResizeObserver !== 'undefined') {
    pathBreadcrumbResizeObserver = new ResizeObserver(scrollPathToCurrentFolder)
    if (pathBreadcrumbViewportRef.value) {
      pathBreadcrumbResizeObserver.observe(pathBreadcrumbViewportRef.value)
    }
  }
  scrollPathToCurrentFolder()
})
...
.path-breadcrumb-viewport::-webkit-scrollbar {
  display: none;
}
