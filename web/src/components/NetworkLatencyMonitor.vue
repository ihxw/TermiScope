<template>
  <a-card :title="t('network.latencyTitle')" :bordered="false" size="small" style="margin-top: 12px">
    ...
    <DataLoadError
      ...
      :loading="chartLoading"
      @retry="fetchChartData({ allowEmpty: true })"
    />
    <a-spin v-else :spinning="chartLoading">
      <a-empty
...
function getVisibleTimeWindow(seriesData, requestedWindow) {
   const timestamps = seriesData
      .flatMap((points) => points.map((point) => point[0]?.getTime()))
      .filter(Number.isFinite)

   if (timestamps.length === 0) return requestedWindow

   const requestedMin = requestedWindow.min.getTime()
   const requestedMax = requestedWindow.max.getTime()
   const requestedSpan = requestedMax - requestedMin
   const dataMin = Math.min(...timestamps)
   const dataMax = Math.max(...timestamps)
   const dataSpan = dataMax - dataMin

   // Keep the selected range when history nearly fills it. For new or sparse
   // tasks, fit the axis to the available history instead of leaving it at the right edge.
   if (dataSpan >= requestedSpan * 0.8) return requestedWindow

   const padding = Math.max(dataSpan * 0.03, Math.min(requestedSpan * 0.01, 60_000))
   return {
      min: new Date(dataMin - padding),
      max: new Date(dataMax + padding),
   }
}
...
const updateChart = () => {
   ...
   const visibleWindow = getVisibleTimeWindow(displayData, chartWindow.value)
   ...
   const option = {
      tooltip: {
         ...
         formatter: (params) => {
             ...
             params.forEach((param) => {
				 ...
				 result += `${param.marker} ${escapeHtml(param.seriesName)}: <b>${escapeHtml(value)}</b>`
                 if (timeStr) {
					 result += ` <span style="color:#888;font-size:12px;margin-left:8px">(${escapeHtml(timeStr)})</span>`
                 }
                 result += '<br/>'
             })
             return result
      ...
      xAxis: {
         type: 'time',
         boundaryGap: false,
         min: visibleWindow.min.getTime(),
         max: visibleWindow.max.getTime()
      },
      yAxis: {
         type: 'value',
         name: 'Latency (ms)',
