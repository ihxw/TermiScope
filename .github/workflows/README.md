...
| 工具 | 用途 | 说明 |
| --- | --- | --- |
| **Go** | 编译后端/Agent | 由 `scripts/ci/setup_go.sh` 按 `go.mod` 安装（当前需 **≥ 1.25.5**）；自建 Runner 不用 `actions/setup-go` |
...
| `GOCACHE is not defined` / `neither $XDG_CACHE_HOME nor $HOME` | 自建 Runner 无 `HOME`，`actions/setup-go` 的 Node 进程无法跑 `go env` | 使用 `setup_go.sh`（已替代 setup-go）；并确保 GitHub 上的 workflow 已同步（勿保留旧版 setup-go） |
